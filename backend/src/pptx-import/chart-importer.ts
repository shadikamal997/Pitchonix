...
export function importChart(pkg: OoxmlPackage, chartPath: string): ImportedChart | null {
  ...
  const plotKey = Object.keys(plot).find(
    (k) => k.endsWith('Chart') && k !== 'c:catAx' && k !== 'c:valAx',
  );
  ...
  const axes = {
    x: {
      title:
        extractText(plot['c:catAx']?.[0]?.['c:title'] || plot['c:catAx']?.['c:title']) || undefined,
    },
    y: {
      title:
        extractText(plot['c:valAx']?.[0]?.['c:title'] || plot['c:valAx']?.['c:title']) || undefined,
    },
  };
...
function pickCategories(seriesNodes: any[]): string[] {
  for (const s of seriesNodes) {
    const cats =
      s['c:cat']?.['c:strRef']?.['c:strCache']?.['c:pt'] ||
...
function buildSeries(s: any): { name: string; values: number[]; color?: string } {
  const name =
    extractText(s['c:tx']) || s['c:tx']?.['c:strRef']?.['c:strCache']?.['c:pt']?.[0]?.['c:v'] || '';
  const pts =
    s['c:val']?.['c:numRef']?.['c:numCache']?.['c:pt'] || s['c:val']?.['c:numLit']?.['c:pt'];
  ...
  return {
    name: String(name),
    values,
    color: color ? `#${String(color).toUpperCase()}` : undefined,
  };
}
