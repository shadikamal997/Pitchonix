...
export function importMaster(pkg: OoxmlPackage, masterPath: string): ImportedMaster | null {
  ...
  for (const sp of sps) {
    ...
    if (type === 'ftr') {
      hasFooter = true;
      elements.push({ kind: 'footer', text, ...box });
    } else if (type === 'sldNum') {
      hasPageNumbers = true;
      elements.push({ kind: 'pageNumber', text, ...box });
    } else if (type === 'dt') {
      elements.push({ kind: 'header', text, ...box });
    } else if (type === 'pic') {
      elements.push({ kind: 'logo', ...box });
    } else {
      elements.push({ kind: 'placeholder', text, ...box });
    }
  }
  ...
  return {
    ...
    slots: {
      ...
      logos: elements.some((e) => e.kind === 'logo'),
    },
    defaultStyles,
    elements,
  };
