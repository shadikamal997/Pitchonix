...
export class PptxImportService {
  ...
  /** Phase Ω.1E — record ownership for every /uploads asset referenced by an
   *  imported deck (extracted images + OLE embeddings), owned by the project. */
  private async recordImportedAssets(parsed: any, projectId: string, deckId: string) {
    ...
    const re = /\/uploads\/(?:images|embeddings)\/[A-Za-z0-9._-]+/g;
    const scan = (v: any) => {
      if (v == null) return;
      const s = typeof v === 'string' ? v : JSON.stringify(v);
      let m;
      while ((m = re.exec(s)) !== null) urls.add(m[0]);
    };
    ...
    for (const mm of parsed.masters || []) for (const me of mm.elements || []) scan(me);
    const nodePath = await import('path');
    for (const publicPath of urls) {
      await this.uploadedAssets.record({
        userId: project.userId,
        publicPath,
        storagePath: nodePath.join(process.cwd(), publicPath),
        module: 'pptx_import',
        projectId,
        documentId: deckId,
        visibility: 'private',
  ...
  parseBuffer(buffer: Buffer): PptxImportResult {
    ...
    try {
      pkg = new OoxmlPackage(buffer);
    } catch (e: any) {
      throw new BadRequestException(`Invalid PPTX (not a zip): ${e?.message}`);
    }
    ...
    const themes: ImportedTheme[] = pkg
      .list(/^ppt\/theme\/theme\d+\.xml$/)
      .map((p) => importTheme(pkg, p)!)
      .filter(Boolean);
    const masters: ImportedMaster[] = pkg
      .list(/^ppt\/slideMasters\/slideMaster\d+\.xml$/)
      .map((p) => importMaster(pkg, p)!)
      .filter(Boolean);
    const layouts: ImportedLayout[] = pkg
      .list(/^ppt\/slideLayouts\/slideLayout\d+\.xml$/)
      .map((p) => importLayout(pkg, p)!)
      .filter(Boolean);

    // Phase 38.2A — SmartArt (parse every drawing.xml).
    const smartArt: ImportedSmartArt[] = pkg
      .list(/^ppt\/diagrams\/drawing\d+\.xml$/)
      .map((p) => importSmartArt(pkg, p)!)
      .filter(Boolean);
    ...
    const slides: ImportedSlide[] = slidePaths.map((p, i) => {
      ...
      try {
        ...
        /* keep import going even if resolution fails */
      }
      ...
      totalText += out.elements.filter(
        (e) => e.type === 'paragraph' || e.type === 'heading',
      ).length;
    ...
    try {
      longTail = scanLongTailExtensions(pkg);
    } catch {
      longTail = undefined;
    }
  ...
  async importIntoProject(
    buffer: Buffer,
    projectId: string,
  ): Promise<{ deckId: string; report: ImportReport }> {
    ...
    // Phase Ω.1E — record ownership for extracted images / OLE embeddings.
    await this.recordImportedAssets(parsed, projectId, deck.id);

    // Phase Ω.CONTENT.2F — content ledger: record every parsed PPTX node as
    // imported → rendered → saved. Best-effort; never fail import over it.
    try {
      await this.pptxImportLedger.recordImport(deck.id, parsed.slides);
    } catch (err) {
      this.logger.warn(
        `PPTX import content ledger skipped for deck ${deck.id}: ${(err as Error).message}`,
      );
    }
  ...
  private parseSlide(
    ...
    const walk = (node: any, groupId?: string) => {
      ...
      for (const pic of asArray(node['p:pic']))
        this.handlePic(pic, elements, order++, slidePath, pkg, mediaMap, groupId);
      for (const gf of asArray(node['p:graphicFrame']))
        this.handleGraphicFrame(
          gf,
          elements,
          order++,
          slidePath,
  ...
  private handleGraphicFrame(
    ...
    const box = off && ext ? readBox({ 'a:xfrm': { 'a:off': off, 'a:ext': ext } }) : null;
    if (!box) return;
    ...
    if (chartRef) {
      ...
      if (chartPath) {
        ...
        if (c) {
          if (c.skipped)
            warnings.push(
              `Chart "${c.title || 'untitled'}" mapped to bar (unsupported OOXML kind).`,
            );
          out.push({
            type: 'chart',
            order,
...
function buildCompatReport(c: {
  ...
  push(
    ...
    c.charts,
    imported,
    c.charts > 0 ? 'mapped to closest Pitchonix chart kind (3D / surface flattened)' : undefined,
  );
  ...
  if (c.smartArt > 0) {
    partial.push({
      kind: 'smartArt',
      count: c.smartArt,
      note: 'Imported as grouped flat shapes (live edit semantics not preserved)',
    });
  }
  ...
  if (c.ole > 0) {
    partial.push({
      kind: 'oleObjects',
      count: c.ole,
      note: 'Imported as attachment cards (open in original application)',
    });
  }
  // Theme inheritance resolution scope.
  if (c.resolved > 0) {
    partial.push({
      kind: 'themeInheritance',
      count: c.resolved,
      note: 'Resolved master→layout→slide tokens; advanced inheritance edge-cases not 100%',
    });
  }
