...
function layoutShapes(kind: SmartArtKind, nodes: SmartArtNode[]): SmartArtShape[] {
  ...
  switch (kind) {
    ...
    case 'orgchart': {
      ...
      for (let i = 0; i < kids.length; i++) {
        ...
        out.push({
          ...
          text: kids[i].text,
          fill: PALETTE.accent,
          textSize: textSizeFor(kids.length, kidW),
          textAlign: 'center',
        });
      ...
      if (grandKids.length > 0) {
        ...
        for (let j = 0; j < grandKids.length; j++) {
          ...
          out.push({
            ...
            w: gW,
            h: gH,
            text: grandKids[j].text,
            fill: PALETTE.primary,
            textSize: textSizeFor(grandKids.length, gW),
            textAlign: 'center',
          });
        }
      }
    ...
    default: {
      ...
      const r = 35;
      const spokeR = Math.max(6, Math.min(10, 60 / Math.max(1, spokes.length)));
      for (let i = 0; i < spokes.length; i++) {
        ...
        out.push({
          ...
          text: spokes[i].text,
          fill: PALETTE.secondary,
          textSize: textSizeFor(spokes.length, spokeR * 2),
          textAlign: 'center',
        });
        out.push({
          kind: 'connector',
