...
export async function exportDeckToPptx(deck: RenderDeckInput): Promise<Buffer> {
  ...
  for (const slide of deck.slides) {
    ...
    for (const el of sorted) {
      try {
        addElement(pptx, ps, el, chartImages);
      } catch (err) {
    ...
    if (slide.speakerNotes && slide.speakerNotes.trim().length > 0) {
      try {
        ps.addNotes(slide.speakerNotes);
      } catch (e) {
        /* pptxgenjs swallows missing */
      }
    }
    ...
    if (slide.transition && (slide.transition as any).effect) {
      try {
        ...
        if (typeof psAny.transition === 'function') {
          psAny.transition({
            type: pptxTransitionType(t.effect),
            duration: (t.duration ?? 400) / 1000,
          });
        }
      } catch {
        /* unsupported in this pptxgenjs version */
      }
    }
  }
...
async function prerenderChartImages(deck: RenderDeckInput): Promise<ChartImageMap> {
  ...
  for (const slide of deck.slides) {
    for (const el of slide.elements || []) {
      ...
      jobs.push(
        renderChartToDataUrl(el)
          .then((url) => {
            if (url) map.set(el.id, url);
          })
          .catch((err) => {
            console.warn(
              `[pptx-export] chart pre-render failed for ${el.id}:`,
...
function pptxTransitionType(effect: string): string {
  switch (effect) {
    ...
    case 'cover':
      return 'cover';
    case 'morph':
      return 'fade'; // morph isn't a v3 native; fade is the closest fallback
    default:
      return 'fade';
  }
}
...
function addElement(_pptx: any, slide: any, el: SlideElementDTO, chartImages: ChartImageMap) {
  switch (el.type) {
    case 'heading':
      return addText(slide, el, { fontSize: 32, bold: true });
    case 'subheading':
      return addText(slide, el, { fontSize: 18, bold: false, color: '475569' });
    case 'paragraph':
      return addText(slide, el, { fontSize: 14 });
    case 'caption':
      return addText(slide, el, { fontSize: 11, color: '6B7280' });
    case 'label':
      return addText(slide, el, { fontSize: 11, bold: true, color: '475569' });
    case 'cta':
      return addCta(slide, el);
    case 'quote':
      return addQuote(slide, el);
    case 'testimonial':
      return addTestimonial(slide, el);
    case 'bulletList':
      return addBulletList(slide, el, false);
    case 'numberedList':
      return addBulletList(slide, el, true);
    case 'metric':
      return addMetric(slide, el);
    case 'kpi':
      return addKpi(slide, el);
    case 'chart':
      return addChart(slide, el, chartImages);
    case 'table':
      return addTable(slide, el);
    case 'image':
      return addImage(slide, el);
    case 'logo':
      return addImage(slide, el);
    case 'shape':
      return addShape(slide, el);
    case 'line':
      return addLine(slide, el);
    case 'divider':
      return addDivider(slide, el);
    case 'footer':
      return addText(slide, el, { fontSize: 10, color: '94A3B8' });
    case 'pageNumber':
      return addText(slide, el, { fontSize: 10, color: '94A3B8', align: 'right' });
    case 'pricingCard':
      return addPricing(slide, el);
    case 'featureGrid':
      return addFeatureGrid(slide, el);
    case 'timeline':
      return addTimeline(slide, el);
    case 'roadmap':
      return addRoadmap(slide, el);
    case 'fundsAllocation':
      return addFundsAllocation(slide, el);
    case 'teamCard':
      return addTeam(slide, el);
    case 'swot':
      return addSwot(slide, el);
    case 'comparison':
      return addComparison(slide, el);
    case 'processSteps':
      return addProcess(slide, el);
    default:
      return addText(slide, el, { fontSize: 14 });
  }
}
...
function pickTextStyle(
  el: SlideElementDTO,
  base: { fontSize?: number; bold?: boolean; color?: string; align?: 'left' | 'center' | 'right' },
) {
...
function addText(
  ...
  const richRuns = Array.isArray(c.runs)
    ? (c.runs as any[]).filter((r) => r && typeof r.text === 'string')
    : null;
  if (richRuns && richRuns.length > 0) {
    const blocks = richRuns.map((r) => ({
      ...
      options: {
        ...
        color:
          r.color && typeof r.color === 'string' && !r.color.startsWith('scheme:')
            ? stripHash(r.color)
            : ts.color,
...
function addCta(slide: any, el: SlideElementDTO) {
  ...
  slide.addShape('roundRect', {
    ...rect(el),
    fill: { color: fill },
    line: variant === 'outline' ? { color: '16A34A', width: 2 } : { color: fill },
    rectRadius: 0.08,
  });
...
function addQuote(slide: any, el: SlideElementDTO) {
  ...
  if (c.attribution) {
    slide.addText(`— ${c.attribution}${c.role ? `, ${c.role}` : ''}`, {
      x: r.x,
      y: r.y + r.h - 0.4,
      w: r.w,
      h: 0.4,
...
function addTestimonial(slide: any, el: SlideElementDTO) {
  ...
  if (c.role || c.company) {
    slide.addText([c.role, c.company].filter(Boolean).join(' · '), {
      x: r.x + 0.2,
      y: r.y + r.h - 0.45,
      w: r.w - 0.4,
      h: 0.3,
...
function addMetric(slide: any, el: SlideElementDTO) {
  ...
  slide.addText(`${c.value || '—'}${c.unit ? ` ${c.unit}` : ''}`, {
    x: r.x,
    y: r.y,
    w: r.w,
    h: r.h * 0.55,
    fontSize: fit?.fontSize ?? 36,
    bold: true,
    color: accent,
    valign: 'middle',
    fit: 'shrink',
  ...
  if (c.delta) {
    const dc =
      c.deltaDirection === 'up' ? '16A34A' : c.deltaDirection === 'down' ? 'DC2626' : '6B7280';
...
function addKpi(slide: any, el: SlideElementDTO) {
  ...
  slide.addText(c.value || '—', {
    ...
    w: r.w - 0.3,
    h: r.h * 0.55,
    fontSize: Math.min(fit?.fontSize ?? 28, 40),
    bold: true,
    color: '16A34A',
    fit: 'shrink',
    margin: 0,
...
function addFundsAllocation(slide: any, el: SlideElementDTO) {
  ...
  if (items.length === 0) {
    slide.addText('Funding allocation', {
      ...r,
      fontSize: 12,
      color: '6B7280',
      margin: 0,
...
function addTable(slide: any, el: SlideElementDTO) {
  ...
  const tableRows = [
    headers.map((h) => ({
      ...
      options: {
        bold: true,
        fill: { color: stripHash(h?.fill || 'F1F5F9') },
        color: stripHash(h?.color || '0F172A'),
  ...
  slide.addTable(tableRows, {
    ...rect(el),
    border: { color: stripHash(c.borders?.color || 'E2E8F0'), pt: c.borders?.width || 1 },
    fontSize: 11,
  });
}
...
function addImage(slide: any, el: SlideElementDTO) {
  ...
  if (c.src && /^(https?:|data:)/.test(c.src)) {
    slide.addImage({
      path: c.src,
      ...r,
      sizing: { type: c.fit === 'contain' ? 'contain' : 'cover', w: r.w, h: r.h },
    });
  } else if (c.src) {
...
function addShape(slide: any, el: SlideElementDTO) {
  ...
  slide.addShape(shape, {
    ...
    line: c.stroke
      ? { color: stripHash(c.stroke), width: c.strokeWidth ?? 1 }
      : { type: 'none' as any },
...
function addLine(slide: any, el: SlideElementDTO) {
  ...
  slide.addShape('line', {
    ...rect(el),
    line: {
      color: stripHash(c.stroke || 'CBD5E1'),
...
function addDivider(slide: any, el: SlideElementDTO) {
  ...
  if (c.label) {
    slide.addShape('line', {
      ...
      w: r.w * 0.4,
      h: 0,
      line: { color: stripHash(c.stroke || 'E2E8F0'), width: c.strokeWidth || 1 },
    });
    slide.addText(c.label, {
      x: r.x + r.w * 0.4,
      y: r.y,
...
function addPricing(slide: any, el: SlideElementDTO) {
  ...
  tiers.forEach((t, i) => {
    ...
    slide.addShape('roundRect', {
      ...
      h: r.h,
      fill: { color: 'FFFFFF' },
      line: { color: t.highlight ? '16A34A' : 'E2E8F0', width: t.highlight ? 2 : 1 },
      rectRadius: 0.1,
    });
    ...
    slide.addText(`${t.price || ''}${t.period ? `/${t.period}` : ''}`, {
      x: x + 0.15,
      y: r.y + 0.6,
      w: tw - 0.3,
      h: 0.6,
...
function addSwot(slide: any, el: SlideElementDTO) {
  ...
  cells.forEach((cell) => {
    ...
    const bullets = (cell.items || []).map((it: string) => ({
      text: it,
      options: { bullet: true },
    }));
    if (bullets.length)
...
function addComparison(slide: any, el: SlideElementDTO) {
  ...
  const tableRows = [
    [
      { text: '', options: { bold: true, fill: { color: 'F1F5F9' } } },
      ...cols.map((col, i) => ({
        text: col,
        options: {
          bold: true,
          fill: { color: c.highlightColumn === i ? 'DCFCE7' : 'F1F5F9' },
          color: c.highlightColumn === i ? '14532D' : '0F172A',
        },
      })),
    ],
    ...rows.map((r) => [
      { text: r.feature || '', options: { bold: true, fill: { color: 'FFFFFF' } } },
      ...((r.values || []) as string[]).map((v, i) => ({
        text: v,
        options: { fill: { color: c.highlightColumn === i ? 'F0FDF4' : 'FFFFFF' } },
      })),
    ]),
  ];
