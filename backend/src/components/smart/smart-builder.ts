...
export function resetSmartElementCounter() {
  elementCounter = 0;
}
...
function buildKpiCard(t: FamilyTokens): SlideElementDTO[] {
  return [
    ...
    el(
      'caption',
      { x: 8, y: 82, w: 84, h: 12 },
      { text: 'Tracked monthly, 2026 cohort' },
      bodyStyle(t, 11),
    ),
  ];
}
...
function buildMetricCard(t: FamilyTokens): SlideElementDTO[] {
  return [
    ...
    el(
      'caption',
      { x: 6, y: 78, w: 88, h: 14 },
      { text: 'Net dollar retention, last 4 quarters' },
      bodyStyle(t, 11),
    ),
  ];
}
...
function buildStatBlock(t: FamilyTokens): SlideElementDTO[] {
  ...
  return [0, 1, 2].flatMap((i) => [
    ...
    el(
      'label',
      { x: i * cellW + i + 4, y: 18, w: cellW - 8, h: 10 },
      { text: labels[i] },
      labelStyle(t),
    ),
    el(
      'metric',
      { x: i * cellW + i + 4, y: 32, w: cellW - 8, h: 60 },
      { value: values[i] },
      accentStyle(t, 42),
    ),
  ]);
        ...
        tiers: [
          {
            ...
            price: '$9',
            period: 'mo',
            features: ['Up to 3 seats', 'Core charts', 'Email support'],
          },
          {
            name: 'Pro',
            price: '$29',
            period: 'mo',
            features: ['Unlimited seats', 'All charts', 'Priority support'],
            highlight: true,
          },
          {
            name: 'Team',
            price: '$79',
            period: 'mo',
            features: ['SSO', 'Audit logs', 'SLA', 'Dedicated CSM'],
          },
        ],
      },
      {
        ...
        phases: [
          {
            period: 'Q1',
            phase: 'Foundation',
            bullets: ['Hire engineering', 'Close design partners', 'MVP shipped'],
          },
          {
            period: 'Q2',
            phase: 'Launch',
            bullets: ['Public beta', 'PR push', 'First 100 customers'],
          },
          {
            period: 'Q3',
            phase: 'Scale',
            bullets: ['Enterprise tier', 'Funnel optimisation', 'Series A'],
          },
...
function buildTestimonialCard(t: FamilyTokens): SlideElementDTO[] {
  return [
    el(
      ...
      {
        quote:
          'Decks that used to take a week now ship in an afternoon. The component library kept our brand consistent across 40 sales reps.',
        author: 'Dana Wright',
        role: 'Head of Strategy',
        company: 'Cykel',
      },
...
function buildFundingBlock(t: FamilyTokens): SlideElementDTO[] {
  return [
    ...
    el(
      'shape',
      { x: 0, y: 10, w: 32, h: 90, z: -1 },
      { kind: tokens(t).rect, fill: t.surface, stroke: t.border, strokeWidth: t.strokeWidth },
      { borderRadius: t.radius },
    ),
    el(
      'shape',
      { x: 34, y: 10, w: 32, h: 90, z: -1 },
      { kind: tokens(t).rect, fill: t.surface, stroke: t.border, strokeWidth: t.strokeWidth },
      { borderRadius: t.radius },
    ),
    el(
      'shape',
      { x: 68, y: 10, w: 32, h: 90, z: -1 },
      { kind: tokens(t).rect, fill: t.surface, stroke: t.border, strokeWidth: t.strokeWidth },
      { borderRadius: t.radius },
    ),
...
function buildMarketOpportunity(t: FamilyTokens): SlideElementDTO[] {
  return [
    ...
    el(
      'heading',
      { x: 4, y: 18, w: 52, h: 14 },
      { text: '$48B total addressable market' },
      headingStyle(t, 22),
    ),
    el(
      'paragraph',
...
function buildExecutiveSummary(t: FamilyTokens): SlideElementDTO[] {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    out.push(
      el('label', { x: x + 2, y: 40, w: cellW - 4, h: 6 }, { text: labels[i] }, labelStyle(t)),
    );
    out.push(
      el(
        'metric',
        { x: x + 2, y: 48, w: cellW - 4, h: 24 },
        { value: values[i] },
        accentStyle(t, 32),
      ),
    );
  ...
  return [
    ...
    el(
      ...
      },
      {
        fontFamily: t.fontHeading,
        color: t.accent,
        letterSpacing: t.numberLetterSpacing,
...
function buildFinancialDashboard(t: FamilyTokens): SlideElementDTO[] {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    out.push(
      el('metric', { x: x + 1, y: 20, w: 15, h: 18 }, { value: values[i] }, accentStyle(t, 26)),
    );
  }
  out.push(
    el(
      ...
      {
        type: 'dualAxis',
        title: 'Revenue and EBITDA, 2024 – 2028',
...
function buildCaseStudyBlock(t: FamilyTokens): SlideElementDTO[] {
  ...
  const quads = [
    ...
    {
      label: 'Solution',
      body: 'Centralised component library + master elements for legal footers.',
    },
  ...
  for (let i = 0; i < 4; i++) {
    ...
    out.push(
      el('label', { x: x + 3, y: y + 4, w: 42, h: 6 }, { text: quads[i].label }, labelStyle(t)),
    );
    out.push(
      el(
        'paragraph',
        { x: x + 3, y: y + 14, w: 42, h: 30 },
        { text: quads[i].body },
        bodyStyle(t, 13),
      ),
    );
...
function buildHeroStatement(t: FamilyTokens): SlideElementDTO[] {
  ...
  return [
    ...
    el(
      'shape',
      { x: 8, y: 66, w: 12, h: 0.6, z: 0 },
      { kind: 'rect', fill: t.accent },
      { borderRadius: 0 },
    ),
    el(
      'paragraph',
...
function buildProblemStatement(t: FamilyTokens): SlideElementDTO[] {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    out.push(
      el('label', { x: 4, y: y + 3, w: 8, h: 10 }, { text: `0${i + 1}` }, accentStyle(t, 18)),
    );
    out.push(
      el('paragraph', { x: 13, y: y + 4, w: 84, h: 11 }, { text: pains[i] }, bodyStyle(t, 13)),
    );
  }
  out.push(
    el(
...
function buildSolutionStatement(t: FamilyTokens): SlideElementDTO[] {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    out.push(
      el('label', { x: x + 3, y: 38, w: 26, h: 8 }, { text: props[i].title }, accentStyle(t, 20)),
    );
    out.push(
      el('paragraph', { x: x + 3, y: 52, w: 26, h: 24 }, { text: props[i].body }, bodyStyle(t, 13)),
    );
  }
  out.push(
    el(
...
export function buildSmartComponentTree(
  family: { tokens: FamilyTokens },
