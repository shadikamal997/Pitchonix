...
export class BusinessLogicValidator {
  validate(input: WizardInput, slides: SlideContent[]): BusinessLogicReport {
    ...
    // ── Business model ───────────────────────────────────────────────────
    const hasBusinessModelSlide =
      present.has(SlideType.BUSINESS_MODEL) || present.has(SlideType.REVENUE_MODEL);
    ...
    // ── SWOT (strategy only) ─────────────────────────────────────────────
    const hasSwot =
      !!s?.swot && Object.values(s.swot).some((arr) => Array.isArray(arr) && arr.length > 0);
    ...
    if (doc === 'sales_deck') {
      const hasRoiProof =
        present.has(SlideType.CASE_STUDY) || present.has(SlideType.TRACTION) || hasKpis;
      if (!hasRoiProof) {
        sev(
          'missing-roi',
          'warn',
...
const requiresCompetitors = (doc: string) =>
  ['pitch_deck', 'business_plan', 'sales_deck', 'marketing_plan', 'strategy_presentation'].includes(
    doc,
  );
...
const requiresAsk = (doc: string) =>
  ['pitch_deck', 'partnership_proposal', 'board_meeting_deck', 'proposal', 'sales_deck'].includes(
    doc,
  );
