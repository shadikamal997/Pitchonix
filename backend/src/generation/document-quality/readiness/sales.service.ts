...
export class SalesReadinessEngine {
  score(input: WizardInput, slides: SlideContent[]): ReadinessReport {
    ...
    const painScore =
      painText.length >= 60 ? 100 : painText.length >= 30 ? 70 : painText.length >= 10 ? 40 : 0;
    criteria.push({
      key: 'customer-pain',
      label: 'Customer pain',
      score: painScore,
    ...
    const fitScore =
      solText.length >= 80 && diffText.length >= 30
        ? 100
        : solText.length >= 50
          ? 70
          : solText.length >= 20
    ...
    const testimonialScore = hasCaseStudy
      ? 100
      : /testimonial|customer|client|quote/i.test(input.traction || '')
        ? 60
        : 0;
    criteria.push({
      key: 'testimonials',
    ...
    criteria.push({
      ...
      score: ctaScore,
      reason:
        ctaScore < 70 ? 'Add an explicit next-step CTA (demo / signature / pilot)' : undefined,
    });
