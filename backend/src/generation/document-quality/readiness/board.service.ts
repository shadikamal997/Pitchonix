...
export class BoardReadinessEngine {
  score(input: WizardInput, slides: SlideContent[]): ReadinessReport {
    ...
    criteria.push({
      key: 'kpi-completeness',
      label: 'KPI completeness',
      score: kpiScore,
    ...
    const finFields = [f?.revenue, f?.costs, f?.grossMargin, f?.burnRate, f?.runway].filter(
      Boolean,
    ).length;
    ...
    criteria.push({
      ...
      score: riskScore,
      reason:
        riskScore < 70 ? 'Add a risks slide identifying top 3–5 risks + mitigations' : undefined,
    });
    ...
    // ── Decisions required ──────────────────────────────────────────────
    const hasDecision =
      types.has(SlideType.ASK) || /decision|approve|sign-off|vote/i.test(input.desiredAction || '');
    criteria.push({
      key: 'decisions',
    ...
    const weights: Record<string, number> = {
      'kpi-completeness': 0.3,
      'financial-completeness': 0.3,
      risks: 0.15,
      actions: 0.15,
      decisions: 0.1,
    };
    const total = Math.round(criteria.reduce((acc, c) => acc + c.score * (weights[c.key] ?? 0), 0));
