  ...
  score(blueprints: SlideBlueprint[], documentType: string): StructureQualityScore {
    ...
    if (blueprints.length === 0) {
      return {
        ...
        blockDiversityScore: 0,
        paragraphRatioScore: 0,
        informationDensityScore: 0,
    ...
    if (distinctKinds < 4)
      notes.push(`Only ${distinctKinds} distinct block kinds in deck — increase variety`);
    ...
    if (dominance > 0.4)
      notes.push(`Dominant block kind appears in ${Math.round(dominance * 100)}% of blocks`);
    ...
    if (paragraphRatio > 0.4)
      notes.push(`Deck is paragraph-heavy (${Math.round(paragraphRatio * 100)}%)`);

    // 4. Information density: avg dataDensity across slides
    const avgDensity =
      blueprints.reduce((s, bp) => s + bp.profile.dataDensity, 0) / blueprints.length;
    const informationDensityScore = clamp(avgDensity, 0, 100);
    if (avgDensity < 30)
      notes.push(`Low information density (avg ${Math.round(avgDensity)}) — add structured data`);
    ...
    const investorReadinessScore = clamp(
      (preferredPresent / Math.max(1, rule.prefer.length)) * 100,
      0,
      100,
    );
    if (preferredPresent < rule.prefer.length / 2) {
      notes.push(
        `Missing ${rule.prefer.length - preferredPresent} preferred block kinds for ${documentType}`,
      );
    }
    ...
    if (visualPerSlide < 0.5)
      notes.push(`Only ${visualPerSlide.toFixed(1)} visual blocks per slide on average`);

    // Weighted total
    const total = clamp(
      visualDiversityScore * 0.15 +
        blockDiversityScore * 0.15 +
        paragraphRatioScore * 0.2 +
