...
export class ContentStructureAnalyzer {
  ...
  analyze(slide: SlideContent, input: WizardInput): ContentStructureProfile {
    ...
    const dataDensity = this.computeDataDensity({
      numbers,
      people,
      pricingTiers,
      phases,
  ...
  extractNumbers(text: string): ExtractedNumber[] {
    ...
    const countRe =
      /\b([\d,]+(?:\.\d+)?)([KMBkmb]?)\s+(users?|customers?|clients?|companies|countries?|partners?|downloads?|installs?|subscribers?|members?)\b/g;
  ...
  extractPeople(input: WizardInput, slide: SlideContent): ExtractedPerson[] {
    ...
    if (input.structured?.teamMembers && input.structured.teamMembers.length > 0) {
      return input.structured.teamMembers
        .map((m) => ({
          name: m.name,
          role: m.role,
    ...
    const c = (slide.content || {}) as any;
    const raw = Array.isArray(c.members) ? c.members : Array.isArray(c.team) ? c.team : null;
  ...
  extractPricingTiers(input: WizardInput, slide: SlideContent): ExtractedPricingTier[] {
    ...
    if (input.structured?.pricingTiers && input.structured.pricingTiers.length > 0) {
      return input.structured.pricingTiers
        .map((t) => ({
          name: t.name,
          price: t.price,
          features: t.features || [],
    ...
    const segs = pricingText
      .split(/[;\n]|(?:\s\/\s)/)
      .map((s) => s.trim())
      .filter(Boolean);
    const tiers: ExtractedPricingTier[] = [];
    const tierNameRe =
      /^(starter|basic|free|pro|professional|business|premium|enterprise|team|growth|scale)\b/i;
  ...
  extractRoadmap(input: WizardInput, slide: SlideContent): ExtractedRoadmapPhase[] {
    ...
    if (input.structured?.roadmapPhases && input.structured.roadmapPhases.length > 0) {
      return input.structured.roadmapPhases
        .map((p) => ({
          phase: p.phase,
          period: p.period,
    ...
    const c = (slide.content || {}) as any;
    const rawArr = Array.isArray(c.phases)
      ? c.phases
    ...
    const phaseSegRe =
      /\b(Q[1-4]\s*\d{2,4}|Year\s*\d|Phase\s*\d|Month\s*\d{1,2}|H[12]\s*\d{2,4}|[A-Z][a-z]+\s+\d{4})\b/g;
  ...
  extractAllocations(
    ...
    if (input.structured?.funding?.allocations && input.structured.funding.allocations.length > 0) {
      return input.structured.funding.allocations
        .map((a) => ({
          category: a.category,
  ...
  extractCompetitors(input: WizardInput): ExtractedCompetitor[] {
    ...
    if (input.structured?.competitors && input.structured.competitors.length > 0) {
      return input.structured.competitors
        .map((c) => ({
          name: c.name,
    ...
    const names = Array.from(
      new Set(text.match(/\b([A-Z][a-zA-Z]+(?:\s[A-Z][a-zA-Z]+)?)\b/g) || []),
    );
  ...
  extractSwot(input: WizardInput): ExtractedSwot | undefined {
    ...
    const strengths = this.pickClauses(haystack, [
      /\bstrength/i,
      /\bunique\b/i,
      /\badvantage\b/i,
      /\bproven\b/i,
    ]);
    const weaknesses = this.pickClauses(haystack, [
      /\bweak/i,
      /\blimitation\b/i,
      /\bchallenge\b/i,
      /\bgap\b/i,
    ]);
    const opportunities = this.pickClauses(haystack, [
      /\bopportunit/i,
      /\bgrowth\b/i,
      /\btrend\b/i,
      /\bemerging\b/i,
  ...
  extractMarketSizing(
    ...
    slide: SlideContent,
    numbers: ExtractedNumber[],
  ): ExtractedMarketSizing | undefined {
    // Phase 28 — Prefer structured wizard input
  ...
  private computeStructureScore(candidates: BlockKind[], density: number): number {
    const nonParagraphCount = candidates.filter(
      (c) => c !== 'paragraph' && c !== 'bulletList',
    ).length;
