...
type PresentationOverflowDestination =
  | 'primarySlide'
  | 'continuationSlide'
  | 'appendixSlide'
  | 'speakerNotes'
...
interface PresentationSourceNode {
  sourceType: string;
  sourceIndex: number;
  sourceText: string;
  destination: PresentationOverflowDestination;
  reason: string;
}
...
function attachContentPreservationLedger(
  input: WizardInput,
  slide: SlideContent,
  elements: SlideElementDTO[] | null,
): SlideElementDTO[] | null {
  if (!elements || elements.length === 0) return elements;

  const sourceNodes = collectPresentationSourceNodes(input, slide);
  if (sourceNodes.length === 0) return elements;

  const renderedText = normalizeLedgerText(
    elements.map((element) => flattenContentText(element.content)).join('\n'),
  );
  const renderedNodes: PresentationSourceNode[] = [];
  const overflowNodes: PresentationSourceNode[] = [];

  for (const sourceNode of sourceNodes) {
    const sourceText = normalizeLedgerText(sourceNode.sourceText);
    if (!sourceText) continue;
    const primaryRendered = renderedText.includes(sourceText);
    if (primaryRendered) {
      renderedNodes.push({
        ...sourceNode,
        destination: 'primarySlide',
        reason: 'Source node appears in generated slide elements.',
      });
    } else {
      overflowNodes.push(sourceNode);
    }
  }

  if (renderedNodes.length === 0 && overflowNodes.length === 0) return elements;

  const ledger = {
    phase: 'Omega.Content.1C',
    slideType: slide.type,
    renderedNodes,
    overflowNodes,
    unusedNodes: [],
    hiddenNodes: [],
    appendixNodes: overflowNodes.filter((node) => node.destination === 'appendixSlide'),
    speakerNotes: overflowNodes
      .filter((node) => node.destination === 'speakerNotes')
      .map((node) => `[${node.sourceType} #${node.sourceIndex + 1}] ${node.sourceText}`),
    sourcePreserved: sourceNodes.length === renderedNodes.length + overflowNodes.length,
  };
...
function collectPresentationSourceNodes(
  input: WizardInput,
  slide: SlideContent,
): PresentationSourceNode[] {
  const nodes: PresentationSourceNode[] = [];
  ...
  const add = (
    ...
    const textValue = cleanCopy(String(sourceText || ''));
    if (!textValue || textValue.length < 2) return;
    const key = `${sourceType}:${normalizeLedgerText(textValue)}`;
    if (seen.has(key)) return;
    seen.add(key);
    const sourceIndex =
      explicitIndex ?? nodes.filter((node) => node.sourceType === sourceType).length;
    nodes.push({ sourceType, sourceIndex, sourceText: textValue, destination, reason });
  };

  (input.structured?.kpis || []).forEach((kpi, index) => {
    add(
      'kpi',
      `${kpi.label}: ${kpi.value}${kpi.trend ? ` (${kpi.trend})` : ''}`,
      'appendixSlide',
      'KPI exceeds visual metric capacity.',
      index,
    );
  });
  valuesFromText(input.traction || '').forEach((kpi, index) => {
    add(
      'kpi',
      `${kpi.label}: ${kpi.value}`,
      'appendixSlide',
      'Extracted traction KPI exceeds visual metric capacity.',
      index,
    );
  });

  (input.structured?.teamMembers || []).forEach((member, index) => {
    add(
      'teamMember',
      `${member.name}: ${member.role}. ${member.experience || member.responsibilities || ''}`,
      'continuationSlide',
      'Team slide has limited visible member capacity.',
      index,
    );
  });
  lines(input.team || '').forEach((line, index) =>
    add(
      'teamMember',
      line,
      'continuationSlide',
      'Team prose exceeds visible member capacity.',
      index,
    ),
  );

  (input.structured?.marketSizing?.drivers || []).forEach((driver, index) => {
    add(
      'marketDriver',
      driver,
      'speakerNotes',
      'Market slide summarizes top drivers visually.',
      index,
    );
  });
  lines(input.marketOpportunity || '').forEach((line, index) =>
    add(
      'marketDriver',
      line,
      'speakerNotes',
      'Market narrative exceeds driver grid capacity.',
      index,
    ),
  );

  (input.structured?.roadmapPhases || []).forEach((phase, index) => {
    add(
      'roadmapMilestone',
      `${phase.period || ''} ${phase.phase}: ${(phase.milestones || []).join('; ')}`,
      'continuationSlide',
      'Roadmap slide shows primary milestones only.',
      index,
    );
  });
  lines(input.roadmap || '').forEach((line, index) =>
    add(
      'roadmapMilestone',
      line,
      'continuationSlide',
      'Roadmap prose exceeds visual roadmap capacity.',
      index,
    ),
  );

  (input.structured?.pricingTiers || []).forEach((tier, index) => {
    add(
      'pricingTier',
      `${tier.name}: ${tier.price}. ${(tier.features || []).join('; ')}`,
      'appendixSlide',
      'Pricing slide shows primary tiers only.',
      index,
    );
  });
  lines([input.revenueModel, input.pricing].filter(Boolean).join('\n')).forEach((line, index) => {
    add(
      'businessModelPoint',
      line,
      'appendixSlide',
      'Business model slide summarizes visible revenue streams.',
      index,
    );
  });

  (input.structured?.funding?.allocations || []).forEach((allocation, index) => {
    add(
      'fundingAllocation',
      `${allocation.category}: ${allocation.percentage ?? allocation.amount ?? ''}`,
      'appendixSlide',
      'Funding allocation chart has limited segment capacity.',
      index,
    );
  });

  const swot = input.structured?.swot;
  [...(swot?.weaknesses || []), ...(swot?.threats || [])].forEach((risk, index) =>
    add('riskItem', risk, 'appendixSlide', 'Risk slide summarizes top mitigation themes.', index),
  );
  [...(swot?.strengths || []), ...(swot?.opportunities || [])].forEach((point, index) =>
    add(
      'strategicPoint',
      point,
      'speakerNotes',
      'Strategic SWOT detail exceeds visible slide capacity.',
      index,
    ),
  );

  lines(input.problem || '').forEach((line, index) =>
    add(
      'problemPoint',
      line,
      'speakerNotes',
      'Problem slide summarizes primary pain points visually.',
      index,
    ),
  );
  lines(input.solution || '').forEach((line, index) =>
    add(
      'solutionFeature',
      line,
      'speakerNotes',
      'Solution slide summarizes primary capabilities visually.',
      index,
    ),
  );
  lines(input.differentiation || '').forEach((line, index) =>
    add(
      'competitiveAdvantage',
      line,
      'appendixSlide',
      'Competition slide summarizes visible differentiators.',
      index,
    ),
  );
  (input.structured?.competitors || []).forEach((competitor, index) => {
    add(
      'competitor',
      `${competitor.name}: ${competitor.strengths || ''} ${competitor.weaknesses || ''}`,
      'appendixSlide',
      'Competition matrix shows primary competitors only.',
      index,
    );
  });
  lines(input.competitors || '').forEach((line, index) =>
    add(
      'competitor',
      line,
      'appendixSlide',
      'Competition prose exceeds visible matrix capacity.',
      index,
    ),
  );

  if (typeof slide.content === 'string') {
    lines(slide.content).forEach((line, index) =>
      add(
        'slideContent',
        line,
        'sourcePreservationLedger',
        'Generic slide content may exceed visual layout capacity.',
        index,
      ),
    );
  }
...
function normalizeLedgerText(value: string): string {
  return cleanCopy(value).toLowerCase().replace(/\s+/g, ' ').trim();
}
...
function flattenContentText(value: any): string {
  ...
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')
    return String(value);
  if (Array.isArray(value)) return value.map((item) => flattenContentText(item)).join('\n');
...
const text = (
  size = 13,
  weight: ElementStyle['fontWeight'] = 400,
  color = palette.ink,
): ElementStyle => ({
...
const card = (x: number, y: number, w: number, h: number, fill = palette.panel, z = 0) =>
  node(
    'shape',
    { x, y, w, h, z },
    { kind: 'roundedRect', fill, stroke: palette.line, strokeWidth: 1 },
    panel(fill),
    'Panel',
  );
...
function conciseNarrative(raw: string | undefined, fallback: string, maxLines = 2): string {
  const meaningful = lines(raw)
    .filter(
      (line) =>
        !/^(market opportunity|tam|sam|som|problem|solution|traction|business model|team|roadmap)$/i.test(
          line,
        ),
    )
    .slice(0, maxLines);
...
function normalizedFundingAllocations(
  input: WizardInput,
  fallback: FundingAllocation[] = [
  ...
  const withNumbers = source.map((item, i) => {
    const category =
      item.category || fallback[i % fallback.length]?.category || `Allocation ${i + 1}`;
  ...
  let normalized = withNumbers.map((item, i) => {
    ...
    if (!percentage) {
      percentage =
        fallback[i % fallback.length]?.percentage ||
        Math.max(5, Math.round(100 / withNumbers.length));
    }
...
function kpis(input: WizardInput) {
  ...
  const seen = new Set<string>();
  return [...structured, ...extracted, ...fallbacks]
    .filter((k) => {
...
function crimsonBase(intent: string, headline: string, subtitle?: string) {
  return [
    ...
    node(
      'shape',
      { x: 4.5, y: 7, w: 0.45, h: 72, z: 1 },
      { kind: 'rect', fill: palette.accent },
      { fill: palette.accent },
      'Crimson spine',
    ),
    node(
      ...
      { x: 8, y: 7, w: 48, h: 4 },
      { text: intent },
      { ...label(8), color: palette.muted },
    ),
    node(
      'heading',
      { x: 8, y: 14, w: 70, h: 14 },
      { text: wordTrunc(headline, 86) },
      {
    ...
    node(
      'shape',
      { x: 8, y: subtitle ? 39 : 31, w: 13, h: 0.6, z: 1 },
      { kind: 'rect', fill: palette.accent },
      { fill: palette.accent, borderRadius: 999 },
      'Rule',
    ),
  ];
}
...
function crimsonKpis(input: WizardInput): CrimsonMetric[] {
  ...
  return ensure<CrimsonMetric>(normalized, fallbacks, 3)
    .concat(fallbacks)
    .filter((k, i, arr) => arr.findIndex((x) => x.label === k.label) === i)
    .slice(0, 4);
}

function crimsonMetricCard(
  x: number,
  y: number,
  w: number,
  h: number,
  ...
  return [
    ...
    node(
      'label',
      { x: x + 2.4, y: y + 2.2, w: w - 4.8, h: 3.4 },
      { text: m.label },
      { ...label(7), color: palette.muted },
    ),
    node(
      'heading',
      { x: x + 2.4, y: y + 6.2, w: w - 4.8, h: h - 10 },
      { text: m.value },
      {
    ...
    ...(m.delta
      ? [
          node(
            'label',
            { x: x + 2.4, y: y + h - 4, w: w - 4.8, h: 3 },
            { text: m.delta },
            { ...label(6.5), color: '#22c55e' },
          ),
        ]
      : []),
  ];
  ...
  return [
    {
      ...
      price: 'SaaS',
      features: ['Per-seat recurring model', 'Team collaboration', 'Predictable ARR'],
      highlight: true,
    },
    {
      id: 'enterprise',
      ...
      price: 'Custom',
      features: ['SSO and governance', 'Dedicated support', 'Custom SLA'],
      highlight: false,
    },
    {
      id: 'usage',
      ...
      price: 'Variable',
      features: ['Pay-as-you-scale', 'API or consumption tier', 'Expansion with usage'],
      highlight: false,
    },
    {
      id: 'services',
      ...
      price: 'One-time',
      features: ['Onboarding and setup', 'Custom integrations', 'Ongoing success'],
      highlight: false,
    },
  ];
}
...
function isPersonName(s: string): boolean {
  ...
  if (
    /\b(is|are|was|be|built|the|a|an|by|with|and|or|for|of|in|our|team|company|founded)\b/i.test(s)
  )
    return false;
  return /^[A-Z]/.test(s);
}
...
function team(input: WizardInput) {
  ...
  if (structured.length > 0) {
    return structured.map((m, i) => ({
      id: `m-${i}`,
      name: m.name,
      role: m.role,
      bio: m.experience || m.responsibilities || 'Relevant operating experience',
    }));
  }
  const parsed = lines(input.team).map((line, i) => {
    ...
    if (isPersonName(candidate)) {
      ...
      return {
        id: `m-${i}`,
        name: candidate,
        role: parts[1] || 'Leadership',
        bio: parts.slice(2).join(' · ') || 'Domain experience and execution ownership',
      };
    }
  ...
  return [
    {
      ...
      name: 'Founder',
      role: 'CEO',
      bio: 'Product vision, fundraising, and customer development',
    },
    {
      id: 'cto',
      name: 'Technical Lead',
      role: 'CTO',
      bio: 'Engineering and platform architecture',
    },
    {
      id: 'growth',
      name: 'Growth Lead',
      role: 'GTM',
      bio: 'B2B SaaS acquisition and enterprise pipeline',
    },
  ];
}
  ...
  return [
    {
      ...
      period: 'Q1',
      phase: 'Foundation',
      bullets: ['Core product hardening', 'Early design partners', 'PMF validation'],
    },
    {
      id: 'q2',
      period: 'Q2',
      phase: 'Launch',
      bullets: ['Public launch', 'First 100 paying customers', 'Revenue repeatability'],
    },
    {
      id: 'q3',
      period: 'Q3',
      phase: 'Scale',
      bullets: ['Enterprise tier', 'Compliance readiness', 'Partner channels'],
    },
    {
      id: 'q4',
      period: 'Q4',
      phase: 'Expand',
      bullets: ['International', 'API ecosystem', 'Next-round milestones'],
    },
  ];
}
...
function competitors(input: WizardInput) {
  ...
  const diffFeatures = [
    ...lines(input.differentiation || '')
      .slice(0, 5)
      .map((d) => wordTrunc(d.split(/[:.]/)[0].trim(), 32))
      .filter(Boolean),
    'Core capability',
    'Ease of use',
    'Output quality',
  ...
  return {
    ...
    rows: diffFeatures.map((feature) => ({
      ...
      values: [
        'Native',
        ...Array.from({ length: colCount - 1 }, (_, j) => compPartials[j % compPartials.length]),
      ],
    })),
  };
}

export function designInvestorSlide(
  input: WizardInput,
  slide: SlideContent,
  family?: string,
): SlideElementDTO[] | null {
  nextId = 0;
  _applyFamily(family);
  let elements: SlideElementDTO[] | null;
  switch (slide.type) {
    case SlideType.COVER:
      if (isCrimsonFamily()) elements = designCoverCrimson(input);
      else if (isCenteredCoverFamily()) elements = designCoverCentered(input);
      else if (isHeroSplitCoverFamily()) elements = designCoverHeroSplit(input);
      else elements = designCover(input);
      break;
    case SlideType.EXECUTIVE_SUMMARY:
      elements = isCrimsonFamily()
        ? designExecutiveSummaryCrimson(input)
        : designExecutiveSummary(input);
      break;
    case SlideType.PROBLEM:
      elements = designProblem(input);
      break;
    case SlideType.SOLUTION:
      elements = designSolution(input);
      break;
    case SlideType.MARKET_OPPORTUNITY:
    case SlideType.MARKET_TRENDS:
      elements = designMarket(input);
      break;
    case SlideType.BUSINESS_MODEL:
    case SlideType.PRICING:
    case SlideType.REVENUE_MODEL:
      elements = designBusinessModel(input);
      break;
    case SlideType.TRACTION:
      elements = designTraction(input);
      break;
    case SlideType.TEAM:
      elements = designTeam(input);
      break;
    case SlideType.ROADMAP:
    case SlideType.GO_TO_MARKET:
      elements = designRoadmap(input);
      break;
    case SlideType.COMPETITION:
      elements = designCompetition(input);
      break;
    case SlideType.FINANCIALS:
    case SlideType.UNIT_ECONOMICS:
      elements = designFinancials(input);
      break;
    case SlideType.ASK:
      elements = designAsk(input);
      break;
    case SlideType.PRODUCT_FEATURES:
    case SlideType.TECHNOLOGY:
      elements = designProduct(input);
      break;
    case SlideType.COMPANY_OVERVIEW:
      elements = designCompanyOverview(input);
      break;
    case SlideType.CASE_STUDY:
      elements = designCaseStudy(input);
      break;
    case SlideType.PARTNERSHIP:
      elements = designPartnership(input);
      break;
    case SlideType.RISKS:
      elements = designRisks(input);
      break;
    case SlideType.APPENDIX:
      elements = designAppendix(input, slide);
      break;
    case SlideType.VISION:
      elements = designVision(input);
      break;
    default:
      elements = designGeneralInvestorSlide(input, slide);
      break;
  }
  return attachContentPreservationLedger(input, slide, elements);
}
...
function designCoverCrimson(input: WizardInput) {
  ...
  const market = marketSizing(input);
  const stage = (input.businessStage || 'Seed')
    .replace(/-/g, ' ')
  ...
  return [
    ...
    node(
      ...
      { borderRadius: 18, fill: 'rgba(239,68,68,0.13)' },
      'Hero seal',
    ),
    node(
      'label',
      { x: 70, y: 21, w: 16, h: 3.5 },
      { text: 'VC READY' },
      { ...label(7), color: palette.accent, textAlign: 'center' as any },
    ),
    node(
      'label',
      { x: 70, y: 27, w: 16, h: 4 },
      { text: 'AI DECK OS' },
      { ...headingStyle(12, palette.ink), textAlign: 'center' as any },
    ),
...
function designExecutiveSummaryCrimson(input: WizardInput) {
  const metrics = [
    ...
    .filter(isUsefulMetric)
    .slice(0, 4);
  const insights = [
    { title: 'Pain', body: 'Teams lose time and consistency building business-critical decks.' },
    {
      title: 'Product',
      body: 'AI generation plus reusable layouts, brand systems, and export parity.',
    },
    {
      title: 'Moat',
      body: 'Templates, components, and workflow data improve with every customer.',
    },
  ];
  return [
    ...crimsonBase(
      'Executive Summary',
      `${input.companyName || 'Pitchonix'} turns deck creation into a design system.`,
      'A concise investor snapshot across pain, product, traction, and market.',
    ),
    ...
    ...insights
      .map((item, i) => [
        ...
        node(
          'label',
          { x: 10 + i * 28.2, y: 64.2, w: 21.4, h: 3 },
          { text: item.title },
          { ...label(7), color: palette.accent },
        ),
    ...
    caption(
      ...
      76,
      4,
      'Evidence blocks are summarized to keep the slide readable; details continue in the following sections.',
    ),
  ];
}
...
function designProblemCrimson(input: WizardInput) {
  ...
  const support = crimsonKpis(input)
    .filter((m) => !/arr|mrr/i.test(m.label))
    .slice(0, 3);

  return [
    ...crimsonBase(
      'Problem',
      'Presentation workflows waste time and break quality at scale.',
      'The pain is recurring, measurable, and visible in every business-critical deck.',
    ),
    ...painPoints
      .map((p, i) => [
    ...
    ...(support.length
      ? support.map((m, i) => crimsonMetricCard(8 + i * 27, 72, 23, 12, m, 15)).flat()
      : []),
    caption(
      8,
      88,
      78,
      4,
      'No placeholder stages: every card contains a real pain statement or it is not rendered.',
    ),
  ];
}
...
function designTractionCrimson(input: WizardInput) {
  const metrics = crimsonKpis(input);
  const mrr = metrics.find((m) => /mrr/i.test(m.label)) ||
    metrics[0] || { value: '€15K', label: 'MRR', delta: '+120% YoY' };
  const supporting = metrics.filter((m) => m !== mrr).slice(0, 3);
  const arrMetric = metrics.find((m) => /arr|mrr|revenue/i.test(m.label)) || mrr;
  const userMetric = metrics.find((m) => /user|mau|customer/i.test(m.label)) ||
    supporting[0] || { value: '5,000', label: 'Users' };
  ...
  return [
    ...crimsonBase(
      'Traction',
      `${mrr.value} ${mrr.label} with visible adoption signals.`,
      conciseNarrative(
        input.traction,
        'The deck now separates current traction, market signal, and investor proof.',
        1,
      ),
    ),
    ...
    node(
      'label',
      { x: 11, y: 69.5, w: 25, h: 3 },
      { text: `${arrMetric.label} + ${userMetric.label} growth` },
      { ...label(7), color: palette.ink },
    ),
    ...[0.38, 0.58, 0.78, 1]
      .map((v, i) => [
        node(
          'shape',
          { x: 13 + i * 9, y: 78 - v * 7, w: 5.5, h: v * 7, z: 2 },
          { kind: 'rect', fill: i === 3 ? palette.accent : 'rgba(239,68,68,0.5)' },
          { fill: i === 3 ? palette.accent : 'rgba(239,68,68,0.5)', borderRadius: 4 },
          'Growth bar',
        ),
        node(
          'label',
          { x: 12 + i * 9, y: 79, w: 7, h: 2 },
          { text: ['Q1', 'Q2', 'Q3', 'Now'][i] },
          { ...label(5.2), color: palette.muted },
        ),
      ])
      .flat(),
    node(
      'shape',
      { x: 14, y: 79, w: 35, h: 0.4, z: 1 },
      { kind: 'rect', fill: 'rgba(255,255,255,0.18)' },
    ...
    ...['Today', 'Signal', 'Proof']
      .map((titleText, i) => {
        ...
        const body =
          i === 0
            ? `${source.value} ${source.label} in the current operating base.`
            : i === 1
              ? 'Growth is supported by repeatable product and demand signals.'
              : 'Traction evidence is investor-readable without duplicate cards.';
        return [
          crimsonPanel(59, 62 + i * 8.4, 31, 7),
          node(
            'label',
            { x: 61, y: 63.2 + i * 8.4, w: 7, h: 2.2 },
            { text: titleText },
            { ...label(5.8), color: palette.accent },
          ),
...
function designCompetitionCrimson(input: WizardInput) {
  ...
  return [
    ...crimsonBase(
      'Competition',
      `${us} wins on design intelligence and export trust.`,
      'Alternatives generate text, ship static templates, or lose fidelity at export.',
    ),
    ...
    crimsonBody(
      ...
      18,
      11,
      'Structured content, smart layouts, brand-aware components, and export fidelity in one pipeline.',
      9,
    ),
    ...competitorsOnly
      .map((name, i) => [
        ...
        node(
          'label',
          { x: 38 + i * 27, y: 45, w: 19, h: 3 },
          { text: wordTrunc(name, 24) },
          { ...label(7), color: palette.muted },
        ),
        crimsonBody(
          38 + i * 27,
          ...
          4,
          i === 0
            ? 'Text-first output with limited structure.'
            : 'Templates without deep presentation logic.',
          8,
        ),
      ])
      .flat(),
    ...
    ...rows
      .map((r, i) => [
        ...
        node(
          'label',
          { x: 38, y: 64 + i * 6, w: 19, h: 2.8 },
          { text: r.feature },
          { ...label(6.4), color: palette.muted },
        ),
        node(
          'label',
          { x: 60, y: 64 + i * 6, w: 12, h: 2.8 },
          { text: r.us },
          { ...label(6.4), color: palette.accent },
        ),
        node(
          'label',
          { x: 75, y: 64 + i * 6, w: 12, h: 2.8 },
          { text: r.alt },
          { ...label(6.4), color: palette.muted },
        ),
      ])
    ...
    caption(
      ...
      76,
      4,
      'No scrollable table: the slide shows the essential comparison at presentation scale.',
    ),
  ];
}
...
function designSolutionCrimson(input: WizardInput) {
  ...
  const steps = [
    {
      title: 'Plan',
      body: 'Narrative strategy, slide intent, and investor story are generated before rendering.',
    },
    {
      title: 'Compose',
      body: 'Blueprints assemble KPI cards, proof blocks, charts, flows, and branded layouts.',
    },
    {
      title: 'Ship',
      body: 'Preview and export stay aligned so the final deck matches what users designed.',
    },
  ];
  return [
    ...crimsonBase(
      'Solution',
      `${company} turns raw ideas into designed investor slides.`,
      'The product combines presentation strategy, component planning, brand systems, and final render fidelity.',
    ),
    ...
    ...steps
      .map((s, i) => [
        node(
          'label',
          { x: 12 + i * 26, y: 40, w: 8, h: 3 },
          { text: `0${i + 1}` },
          { ...label(8), color: palette.accent },
        ),
        node(
          'heading',
          { x: 18 + i * 26, y: 39.5, w: 14, h: 4 },
          { text: s.title },
          headingStyle(13, palette.ink),
        ),
        ...
        ...(i < 2
          ? [
              node(
                'label',
                { x: 36 + i * 26, y: 44, w: 3, h: 3 },
                { text: '→' },
                { ...headingStyle(14, palette.accent) },
              ),
            ]
          : []),
      ])
    ...
    ...steps
      .map((s, i) => [
        ...
        node(
          'label',
          { x: 11 + i * 28, y: 63, w: 18, h: 3 },
          { text: `Capability ${i + 1}` },
          { ...label(7), color: palette.accent },
        ),
        node(
          'heading',
          { x: 11 + i * 28, y: 68, w: 18, h: 5 },
          { text: s.title },
          headingStyle(15, palette.ink),
        ),
...
function designMarketCrimson(input: WizardInput) {
  ...
  return [
    ...crimsonBase(
      'Market',
      `${m.tam} market with ${m.growth} adoption tailwinds.`,
      `${m.sam} serviceable segment · ${m.som} near-term wedge · ${input.industry || 'business software'}`,
    ),
    ...
    ...[0, 1, 2, 3]
      .map((i) => [
        node(
          'shape',
          { x: 13 + i * 9, y: 55 - i * 5, w: 7, h: 0.8, z: 2 },
          { kind: 'rect', fill: palette.accent },
          { fill: palette.accent, borderRadius: 999 },
          'Curve segment',
        ),
        node(
          'shape',
          { x: 16 + i * 9, y: 54 - i * 5, w: 1.3, h: 1.3, z: 3 },
          { kind: 'ellipse', fill: palette.accent },
          { fill: palette.accent },
          'Curve point',
        ),
      ])
    ...
    node(
      ...
      { x: 31, y: 58, w: 14, h: 3 },
      { text: '▲ we are here' },
      { ...label(6.5), color: palette.accent },
    ),
    ...
    node(
      ...
      { x: 65, y: 38, w: 21, h: 8 },
      { text: m.tam },
      { ...headingStyle(32, palette.accent), letterSpacing: palette.numberLetterSpacing },
    ),
    node(
      'label',
      { x: 65, y: 46, w: 18, h: 3 },
    ...
    ...[
      ...
      .map((item, i) => [
        ...
        node(
          'heading',
          { x: 64 + i * 15, y: 56.2, w: 9, h: 4 },
          { text: item.value },
          headingStyle(15, palette.accent),
        ),
        node(
          'label',
          { x: 64 + i * 15, y: 61, w: 9, h: 2.2 },
          { text: item.label },
          { ...label(6), color: palette.muted },
        ),
      ])
      .flat(),
    node(
    ...
    ...drivers
      .map((d, i) => [
        ...
        node(
          'heading',
          { x: 10 + i * 21, y: 77.3, w: 14, h: 3.5 },
          { text: `Trigger ${i + 1}` },
          headingStyle(10, palette.ink),
        ),
...
function designBusinessModelCrimson(input: WizardInput) {
    ...
    [
      {
        ...
        name: 'Starter',
        price: 'Free',
        features: ['Basic templates', 'Watermarked export'],
      },
      { id: 'pro', name: 'Pro', price: '$19', features: ['Premium templates', 'AI generation'] },
      {
        ...
        name: 'Enterprise',
        price: 'Custom',
        features: ['Brand kits', 'Admin controls'],
      },
    ],
    3,
  ).slice(0, 3);
  return [
    ...crimsonBase(
      'Business Model',
      'Recurring revenue expands through teams, templates, and export workflows.',
      conciseNarrative(
        input.revenueModel || input.pricing,
        'Subscription-first model with enterprise expansion.',
        1,
      ),
    ),
    ...tiers
      .map((t, i) => [
        crimsonPanel(
          ...
          24,
          25,
          i === 1 ? 'rgba(239,68,68,0.13)' : 'rgba(39,39,42,0.92)',
        ),
        node(
          'label',
          { x: 11 + i * 28, y: 40, w: 17, h: 3 },
          { text: t.name || `Tier ${i + 1}` },
          { ...label(8), color: i === 1 ? palette.accent : palette.muted },
        ),
        node(
          'heading',
          { x: 11 + i * 28, y: 45, w: 16, h: 7 },
          { text: t.price || 'SaaS' },
          headingStyle(22, palette.ink),
        ),
        ...ensure(t.features || [], ['Recurring plan', 'Team workflow'], 2)
          .slice(0, 2)
          .map((f, j) =>
            node(
              'label',
              { x: 11 + i * 28, y: 54 + j * 4.2, w: 17, h: 3 },
              { text: `✓ ${wordTrunc(f, 34)}` },
              { ...label(6.4), color: palette.ink },
            ),
          ),
      ])
      .flat(),
    ...[
      ...
      .map((m, i) => crimsonMetricCard(8 + i * 28, 70, 24, 13, m, 24))
      .flat(),
  ];
}
...
function designTeamCrimson(input: WizardInput) {
  ...
  const credible = members.map((m, i) => ({
    ...
    bio:
      /experience in:?$/i.test(m.bio || '') || /^(Team|Founding Team)$/i.test(m.bio || '')
        ? [
            'Owns product vision, customer discovery, and fundraising narrative.',
            'Leads AI workflow architecture and editor reliability.',
            'Builds acquisition loops, customer proof, and enterprise pipeline.',
          ][i]
  ...
  return [
    ...crimsonBase(
      'Team',
      `${input.companyName || 'Pitchonix'} is led by builders with execution ownership.`,
      'Focused operating roles, product judgment, and go-to-market accountability.',
    ),
    ...credible
      .map((m, i) => [
        ...
        node(
          'heading',
          { x: 11 + i * 28, y: 39, w: 18, h: 4 },
          { text: m.name },
          headingStyle(14, palette.ink),
        ),
        node(
          'label',
          { x: 11 + i * 28, y: 45, w: 16, h: 3 },
          { text: m.role },
          { ...label(7), color: palette.accent },
        ),
    ...
    node(
      'label',
      { x: 8, y: 69, w: 30, h: 3 },
      { text: 'Why this team can win' },
      { ...label(7), color: palette.accent },
    ),
    ...['Product architecture', 'Investor storytelling', 'Workflow automation']
      .map((s, i) => [
        crimsonPanel(8 + i * 28, 74, 24, 10),
        node(
          'label',
          { x: 11 + i * 28, y: 77, w: 17, h: 3 },
          { text: s },
          { ...label(7), color: palette.ink },
        ),
      ])
  ...
  const phases = [
    {
      period: 'Q1',
      phase: 'Product Proof',
      bullets: ['Harden editor, templates, export, and collaboration loops.'],
    },
    {
      period: 'Q2',
      phase: 'Repeatable GTM',
      bullets: ['Launch growth motions and convert design partners into paid teams.'],
    },
    {
      period: 'Q3',
      phase: 'Enterprise Ready',
      bullets: ['Add controls, governance, and high-fidelity export workflows.'],
    },
    {
      period: 'Q4',
      phase: 'Scale Platform',
      bullets: ['Expand template systems, integrations, and partner channels.'],
    },
  ];
  return [
    ...crimsonBase(
      'Roadmap',
      'Milestones map directly to investor value creation.',
      'The next four quarters de-risk product, revenue, and enterprise readiness.',
    ),
    node(
      'shape',
      { x: 10, y: 41, w: 78, h: 0.5, z: 1 },
      { kind: 'rect', fill: 'rgba(239,68,68,0.55)' },
    ...
    ...phases
      .map((p, i) => [
        node(
          'shape',
          { x: 13 + i * 22, y: 39.7, w: 1.8, h: 1.8, z: 2 },
          { kind: 'ellipse', fill: palette.accent },
          { fill: palette.accent },
          'Road dot',
        ),
        ...
        node(
          'label',
          { x: 10 + i * 21.5, y: 48, w: 14, h: 3 },
          { text: p.period || `Q${i + 1}` },
          { ...label(7), color: palette.accent },
        ),
        node(
          'heading',
          { x: 10 + i * 21.5, y: 52, w: 15, h: 5 },
          { text: wordTrunc(p.phase || `Milestone ${i + 1}`, 30) },
          headingStyle(12, palette.ink),
        ),
        crimsonBody(
          10 + i * 21.5,
          59,
          14.5,
          7,
          wordTrunc((p.bullets || [])[0] || 'Investor milestone', 66),
          7.5,
        ),
      ])
      .flat(),
    ...
    node(
      'label',
      { x: 11, y: 81.5, w: 30, h: 3 },
      { text: 'Execution focus: product proof → repeatable pipeline → enterprise readiness' },
      { ...label(6.5), color: palette.ink },
    ),
    crimsonPanel(51, 79, 39, 8, 'rgba(239,68,68,0.12)'),
    node(
      'label',
      { x: 54, y: 81.5, w: 31, h: 3 },
      { text: 'Each tranche is gated by shipped proof, not vanity milestones.' },
      { ...label(6.5), color: palette.ink },
    ),
  ];
}
...
function designAskCrimson(input: WizardInput) {
  ...
  return [
    ...crimsonBase(
      'Raise',
      `${amount} to ship the next product generation.`,
      conciseNarrative(
        input.fundingAsk,
        'Funding extends runway and converts the roadmap into measurable investor milestones.',
        1,
      ),
    ),
    ...[
      ...
      { q: 'Q2', gate: '250 customers', body: 'Launch growth loops and repeatable onboarding.' },
      {
        q: 'Q3',
        gate: '500 customers',
        body: 'Enterprise controls, export fidelity, and sales proof.',
      },
    ]
      .map((m, i) => [
        ...
        node(
          'label',
          { x: 11, y: 40 + i * 14, w: 7, h: 3 },
          { text: m.q },
          { ...label(7), color: palette.accent },
        ),
        node(
          'label',
          { x: 30, y: 40 + i * 14, w: 18, h: 3 },
          { text: `Gate: ${m.gate}` },
          { ...label(6.5), color: palette.muted },
        ),
    ...
    node(
      ...
      { x: 67, y: 41, w: 20, h: 8 },
      { text: amount },
      { ...headingStyle(36, palette.accent), letterSpacing: palette.numberLetterSpacing },
    ),
    node(
      'label',
      { x: 67, y: 52, w: 18, h: 3 },
    ...
    caption(
      ...
      76,
      4,
      'Milestones gate each tranche: capital is deployed against shipped code and commercial proof.',
    ),
  ];
}
...
function designCover(input: WizardInput) {
  ...
  const market = marketSizing(input);
  const stage = (input.businessStage || 'Series A')
    .replace(/-/g, ' ')
    .replace(/\b\w/g, (c) => c.toUpperCase());
  const badgeParts = [stage, input.industry, market.tam ? `${market.tam} Market` : null].filter(
    Boolean,
  );
  ...
  const logoNode = input.logo
    ? node(
        'logo',
        { x: 75, y: 5, w: 18, h: 8, z: 5 },
        { src: input.logo, name: input.companyName },
        {
          fill: 'transparent',
          stroke: 'transparent',
          borderRadius: 4,
    ...
    : node(
        ...
        {
          ...headingStyle(13, palette.accent),
          textAlign: 'right',
        },
      );
  ...
  return [
    ...
    node(
      'label',
      { x: 6, y: 7, w: 62, h: 5 },
      { text: [input.companyName, input.industry].filter(Boolean).join('  ·  ') || '' },
      label(10),
    ),
...
function isHeroSplitCoverFamily(): boolean {
  ...
  return ['startup-gradient', 'cobalt-impact', 'violet-creative', 'rose-modern'].includes(
    currentFamily,
  );
}
...
function designCoverCentered(input: WizardInput) {
  ...
  const tagline = (() => {
    ...
    return sol && sol.length > 20
      ? wordTrunc(sol, 120)
      : `${input.industry || 'B2B SaaS'} · ${stage}`;
  })();
  ...
  return [
    ...
    input.logo
      ...
      : node(
          ...
          { x: 10, y: 5, w: 80, h: 5 },
          { text: input.companyName || '' },
          { ...headingStyle(13, palette.accent), textAlign: 'center' as any },
        ),
    ...
    node(
      ...
      { x: 8, y: 12, w: 84, h: 22 },
      { text: headline },
      { ...headingStyle(44, palette.ink), textAlign: 'center' as any, lineHeight: 1.0 },
    ),
    // Centered decorative rule pair
    node(
      'shape',
      { x: 38, y: 36, w: 24, h: 0.6, z: 1 },
      { kind: 'rect', fill: palette.accent },
      { fill: palette.accent, borderRadius: 999 },
      'Rule L',
    ),
    ...
    node(
      'paragraph',
      { x: 10, y: 38, w: 80, h: 7 },
      { text: cleanCopy(tagline) },
      { ...text(14, 500, palette.muted), textAlign: 'center' as any },
    ),
    // Centered badge
    node(
      'label',
      ...
      { ...label(10), color: palette.accent, textAlign: 'center' as any },
    ),
...
function designCoverHeroSplit(input: WizardInput) {
  ...
  return [
    ...
    node(
      ...
      { x: 6, y: 14, w: 53, h: 32 },
      { text: headline },
      { ...headingStyle(38, palette.ink), lineHeight: 1.05 },
    ),
    node(
      'shape',
      { x: 6, y: 47, w: 12, h: 0.75, z: 1 },
      { kind: 'rect', fill: palette.accent },
      { fill: palette.accent, borderRadius: 999 },
      'Rule',
    ),
      ...
      { x: 6, y: 49, w: 52, h: 6 },
      {
        text: cleanCopy(
          input.shortDescription
            ? coverTagline
            : wordTrunc(input.solution?.split(/[.!?]\s+/)[0]?.trim() || '', 100),
        ),
      },
    ...
    node(
      ...
      { x: 65, y: 18, w: 26, h: 28 },
      {
        ...
        label: heroKpi.label,
        delta: heroKpi.delta,
        deltaDirection: heroKpi.delta?.includes('-')
          ? ('down' as const)
          : heroKpi.delta
            ? ('up' as const)
            : ('flat' as const),
    ...
    input.logo
      ...
      : node(
          ...
          { x: 65, y: 63, w: 25, h: 8 },
          { text: input.companyName || '' },
          { ...headingStyle(16, palette.accent), textAlign: 'right' as any },
        ),
  ];
}
...
function problemHeroMetric(input: WizardInput): { value: string; label: string } {
  ...
  const timeMatch = prob.match(
    /(\d+(?:\.\d+)?%)\s+of\s+(?:their\s+)?(?:time|workday|work\s+week|working\s+hours)/i,
  );
  ...
  if (pctMatch) {
    ...
    if (/cost|budget|spend|waste|over/.test(ctx))
      return { value: pctMatch[1], label: 'Cost impact' };
...
function problemSecondMetric(input: WizardInput): { value: string; label: string } | null {
  const prob = input.problem || '';
  const costMatch = prob.match(
    /(\$[\d.]+[KMBb]*)\s*(?:per\s+year|annually|a\s+year|\/year|in\s+(?:errors?|losses?|waste))/i,
  );
  if (costMatch) return { value: costMatch[1], label: 'Annual cost' };
  // Any dollar value
  const dollarMatch = prob.match(/(\$[\d.]+[KMBb]*)/i);
  if (dollarMatch) {
    ...
    const lbl = /cost|error|loss|waste/.test(ctx)
      ? 'Cost impact'
      : /revenue|arr|sales/.test(ctx)
        ? null
        : 'Dollars at risk';
...
function getFamilyGroup(): FamilyGroup {
  ...
  if (['startup-gradient', 'violet-creative', 'rose-modern'].includes(currentFamily))
    return 'startup';
...
function designProblem(input: WizardInput) {
  ...
  switch (getFamilyGroup()) {
    case 'luxury':
      return designProblemLuxury(input);
    case 'startup':
      return designProblemStartup(input);
    case 'consulting':
      return designProblemConsulting(input);
    case 'tech':
      return designProblemTech(input);
    default:
      return designProblemInvestor(input);
  }
}
...
function designProblemInvestor(input: WizardInput) {
  ...
  return [
    ...base(
      ...
      (() => {
        const raw =
          input.problem?.split(/[.!?]\s+/)[0]?.trim() ||
          `${input.companyName || 'The market'} faces a solvable, recurring problem.`;
    ...
    ...painPoints.flatMap((p, i) => [
      ...
      node(
        'label',
        { x: 9, y: 37.5 + i * 16, w: 7, h: 5 },
        { text: `0${i + 1}` },
        text(15, 850, palette.accent),
      ),
    ...
    node(
      'metric',
      { x: 63, y: 38, w: 27, h: 14 },
      { value: hero.value, label: hero.label },
      { ...headingStyle(48, palette.accent), letterSpacing: palette.numberLetterSpacing },
    ),
    ...(secondary
      ? [
          node(
            'shape',
            { x: 63, y: 55, w: 27, h: 0.5, z: 1 },
            { kind: 'rect', fill: palette.muted },
            { fill: palette.muted, opacity: 0.25 },
            'Divider',
          ),
          node(
            'metric',
            { x: 63, y: 57, w: 27, h: 12 },
            { value: secondary.value, label: secondary.label },
            {
              ...headingStyle(34, palette.accent2 || palette.accent),
              letterSpacing: palette.numberLetterSpacing,
            },
          ),
        ]
      : [
          paragraph(
            ...
            conciseNarrative(
              input.problem,
              'Every company in this space faces this exact ceiling.',
              2,
            ),
            12,
          ),
...
function designExecutiveSummary(input: WizardInput) {
  ...
  const insightItems = [
    ...lines(
      [input.problem, input.solution, input.differentiation, input.revenueModel]
        .filter(Boolean)
        .join('\n'),
    ),
  ...
  return [
    ...base(
      ...
      wordTrunc(
        input.shortDescription ||
          `${input.companyName} solves ${input.problem?.split(/[.!?]/)[0]?.toLowerCase() || 'a real market problem'}.`,
        65,
      ),
      wordTrunc(
        input.purpose ||
          input.shortDescription ||
          'A concise investment snapshot across pain, product, market, traction, and ask.',
        150,
      ),
    ),
    ...[
      ...
      .slice(0, 4)
      .map((m, i) => [
        card(6 + i * 21.5, 33, 19, 16),
        metric(8 + i * 21.5, 36, 15, 8, m.value, m.label, (m as any).delta),
      ])
      .flat(),
    node(
      'featureGrid',
      ...
      {
        columns: 2,
        items: insightItems.map((item, i) => ({
          id: `summary-${i}`,
          title: ['Pain', 'Product', 'Moat', 'Model'][i] || `Signal ${i + 1}`,
          description: item,
        })),
      },
      panel(),
              ...
              Math.min(
                ...
                Math.round(
                  55 +
                    Math.min(
                      35,
                      ((input.marketOpportunity || '').length +
                        ((input.structured?.marketSizing?.tam as string) || '').length) /
                        4,
                    ),
                ),
              ),
              Math.min(
                ...
                Math.round(
                  55 +
                    Math.min(
                      35,
                      ((input.revenueModel || '') + (input.fundingAsk || '')).length / 4,
                    ),
                ),
              ),
              input.structured?.teamMembers?.length
                ? 88
                : Math.min(92, Math.round(55 + Math.min(35, (input.team?.length || 0) / 4))),
            ],
...
function designSolution(input: WizardInput) {
  ...
  switch (getFamilyGroup()) {
    case 'luxury':
      return designSolutionLuxury(input);
    case 'startup':
      return designSolutionStartup(input);
    case 'consulting':
      return designSolutionConsulting(input);
    case 'tech':
      return designSolutionTech(input);
    default:
      return designSolutionInvestor(input);
  }
}
...
function designSolutionInvestor(input: WizardInput) {
  ...
  const beforeState = wordTrunc(
    conciseNarrative(input.problem, 'Manual, error-prone, expensive process.', 1),
    110,
  );
  ...
  const engineDesc = (() => {
    ...
    return wordTrunc(
      solFirst.length > 20
        ? solFirst
        : `${input.companyName || 'The platform'} automates and delivers with enterprise-grade accuracy`,
      90,
    );
  })();
  const afterState = (() => {
    ...
    if (kpiData.length >= 2) {
      ...
      if (retention)
        return `${retention.value} net retention · ${kpiData[0].value} ${kpiData[0].label} · proven at scale`;
      return `${kpiData[0].value} ${kpiData[0].label} · ${kpiData[1]?.value || ''} ${kpiData[1]?.label || ''}`.trim();
    }
    return wordTrunc(
      conciseNarrative(input.solution, 'Accurate, instant, and enterprise-ready.', 1),
      80,
    );
  })();
  ...
  const diffLines = lines(normalizedDiff).filter(
    (f) => f.length > 8 && !/^\d+[%x×]?$/.test(f.trim()),
  );
  ...
  const capabilityItems = [
    ...
    .map((f, i) => {
      ...
      const rawT = wordTrunc(
        (colon > 0 && colon < 42 ? f.slice(0, colon) : f.split(/\s+/).slice(0, 4).join(' '))
          .replace(/[,.:;]+$/, '')
          .trim(),
        34,
      );
      ...
      const desc =
        rawD.toLowerCase().trim() === rawT.toLowerCase().trim()
  ...
  const solutionSubtitle = (() => {
    ...
    const kpiData = kpis(input);
    if (kpiData.length >= 2)
      return `${kpiData[0].value} ${kpiData[0].label} and ${kpiData[1].value} ${kpiData[1].label} — built on repeatable, scalable foundations.`;
  ...
  return [
    ...
    node(
      'featureGrid',
      { x: 6, y: 59, w: 86, h: 22 },
      { columns: capabilityItems.length >= 3 ? 3 : 2, items: capabilityItems },
      panel(),
    ),
    caption(
      6,
      ...
      86,
      6,
      'Competitive advantages that compound with every enterprise integration.',
    ),
  ];
}
...
function designMarket(input: WizardInput) {
  ...
  switch (getFamilyGroup()) {
    case 'luxury':
      return designMarketLuxury(input);
    case 'startup':
      return designMarketStartup(input);
    case 'consulting':
      return designMarketConsulting(input);
    case 'tech':
      return designMarketTech(input);
    default:
      return designMarketInvestor(input);
  }
}
...
function designMarketInvestor(input: WizardInput) {
  ...
  const cleanedMarketText = (input.marketOpportunity || '')
    ...
    .replace(/\s{2,}/g, ' ')
    .trim();
  const narrative = conciseNarrative(
    cleanedMarketText || undefined,
  ...
  return [
    ...
    node(
      ...
      { x: 9, y: 60, w: 14, h: 3 },
      { text: m.growth },
      { ...headingStyle(20, palette.accent2 || palette.accent) },
    ),
    node(
      'label',
      { x: 24, y: 61, w: 16, h: 3 },
    ...
    node(
      'metric',
      { x: 65, y: 35.5, w: 25, h: 10 },
      { value: m.sam, label: 'SAM' },
      {
        ...headingStyle(36, palette.accent2 || palette.accent),
        letterSpacing: palette.numberLetterSpacing,
      },
    ),
    // SOM — bottom right
    ...
    node(
      'metric',
      { x: 65, y: 55.5, w: 25, h: 10 },
      { value: m.som, label: 'SOM' },
    ...
    node(
      ...
      { x: 6, y: 71, w: 86, h: 23 },
      {
        ...
        items: m.drivers.slice(0, 4).map((d, i) => {
          ...
          return {
            id: `driver-${i}`,
            title: words.replace(/[,.:;]+$/, '').slice(0, 36),
            description: d,
          };
        }),
      },
...
function designBusinessModel(input: WizardInput) {
  ...
  return [
    ...base(
      'Business Model',
      'Multiple revenue streams compound into scalable ARR.',
      conciseNarrative(
        input.revenueModel || input.pricing,
        'Subscription-first model with enterprise expansion.',
      ),
    ...
    metric(
      ...
      kpis(input).find((k) => /margin|gross/i.test(k.label))
        ? 'Gross margin'
        : 'Target gross margin',
    ),
    card(35, 69, 26, 14),
...
function designTraction(input: WizardInput) {
  ...
  const arrValues =
    arrBase > 0
      ? [
          Math.round(arrBase * 0.37),
          Math.round(arrBase * 0.57),
          Math.round(arrBase * 0.79),
          arrBase,
        ]
      : [20, 38, 62, 95];
  const mauValues =
    mauBase > 0
      ? [Math.round(mauBase * 0.4), Math.round(mauBase * 0.58), Math.round(mauBase * 0.78), mauBase]
      : [300, 550, 900, 1400];
  ...
  if (kpiData[0]) {
    ...
    timelineItems.push({
      id: 'tl-0',
      date: 'Today',
      title: `${kpiData[0].value} ${kpiData[0].label}`,
      description: `Current run rate${delta}`,
    });
  }
  if (kpiData[1]) {
    ...
    timelineItems.push({
      id: 'tl-1',
      date: 'Signal',
      title: `${kpiData[1].value} ${kpiData[1].label}`,
      description: `Organic adoption${delta}`,
    });
  }
  if (kpiData[2]) {
    timelineItems.push({
      id: 'tl-2',
      date: 'Proof',
      title: `${kpiData[2].value} ${kpiData[2].label}`,
      description: 'Best-in-class benchmark',
    });
  }
    ...
    timelineItems.push(
      {
        ...
        description: 'Product-market fit validated',
      },
      {
        id: 'tl-1',
        date: 'Growth',
        title: 'Revenue inflection',
        description: 'Paid expansion begins',
      },
      {
        id: 'tl-2',
        date: 'Now',
        title: 'Scale signal',
        description: 'Enterprise accounts entering',
      },
    );
  }
  ...
  return [
    ...base(
      'Traction',
      heroLabel,
      conciseNarrative(input.traction, 'The numbers show early pull and repeatable growth.'),
    ),
    ...data
      .map((m, i) => [
        card(6 + i * 21.5, 33, 19, 17),
        metric(8 + i * 21.5, 34, 15, 14, m.value, m.label, m.delta),
      ])
      .flat(),
    node(
      'chart',
...
function designTeam(input: WizardInput) {
  ...
  switch (getFamilyGroup()) {
    case 'luxury':
      return designTeamLuxury(input);
    case 'startup':
      return designTeamStartup(input);
    case 'consulting':
      return designTeamConsulting(input);
    case 'tech':
      return designTeamTech(input);
    default:
      return designTeamInvestor(input);
  }
}
...
function designTeamInvestor(input: WizardInput) {
  ...
  const teamInsights = [
    {
      id: 'domain',
      title: 'Domain depth',
      description: wordTrunc(
        conciseNarrative(
          input.team,
          `${input.companyName || 'The team'} has category-specific operating history.`,
          1,
        ),
        90,
      ),
    },
    {
      ...
      description: wordTrunc(
        conciseNarrative(input.traction, 'Traction validates that this team can ship and sell.', 1),
        90,
      ),
    },
    {
  ...
  return [
    ...
    node(
      'featureGrid',
      { x: 6, y: teamCardEndY + 2, w: 86, h: Math.min(22, 88 - teamCardEndY) },
      {
        columns: 3,
        items: teamInsights,
      },
    ...
    caption(
      ...
      86,
      6,
      'Investor lens: credible team, clear ownership, and category-specific expertise',
    ),
  ];
}
      ...
      { x: 6, y: 78, w: 42, h: 9 },
      {
        ...
        series: (() => {
          ...
          if (allocs && allocs.length >= 2) {
            return allocs.slice(0, 4).map((a, i) => ({
              name: a.category,
              values: [a.percentage],
...
function designCompanyOverview(input: WizardInput) {
  ...
  return [
    ...base(
      ...
      wordTrunc(
        input.shortDescription ||
          `${input.companyName} — ${input.productService?.split(/[.!?,]/)[0]?.trim() || 'the company at a glance'}`,
        65,
      ),
      wordTrunc(
        input.productService ||
          input.shortDescription ||
          'What the company does, who it serves, and how it wins.',
        150,
      ),
    ),
    node(
        ...
        items: [
          {
            ...
            description: wordTrunc(
              input.targetCustomers ||
                `${input.industry || 'Enterprise'} teams with a clear budget and urgency`,
              70,
            ),
          },
          {
            ...
            title: 'Category',
            description: wordTrunc(
              input.industry || 'B2B SaaS — recurring revenue with enterprise expansion',
              70,
            ),
          },
          {
            ...
            title: 'Wedge',
            description: wordTrunc(
              input.differentiation?.split(/[.!?\n]/)[0]?.trim() ||
                `${input.companyName || 'We'} win on speed, accuracy, and zero integration friction`,
              70,
            ),
          },
          {
            ...
            description: wordTrunc(
              input.businessStage
                ? `${input.businessStage} — product-led with enterprise expansion`
                : 'Product-led adoption with enterprise upsell',
              70,
            ),
          },
        ],
    ...
    node(
      ...
      { x: 60, y: 34, w: 32, h: 38 },
      {
        steps: model.map((tier, i) => ({
          id: tier.id || `model-${i}`,
          title: tier.name,
          description: tier.price || tier.features?.[0] || 'Revenue stream',
        })),
        orientation: 'vertical',
      },
      panel(),
    ...
    caption(
      ...
      86,
      6,
      'Designed as an investor briefing page: category, customer, wedge, and monetization in one scan.',
    ),
  ];
}
...
function designCaseStudy(input: WizardInput) {
  return [
    ...base(
      'Proof Point',
      wordTrunc(
        `${input.companyName || 'A customer'} — measurable outcome from before to after`,
        75,
      ),
      wordTrunc(
        input.traction || 'Before and after: the problem, the intervention, and the result.',
        120,
      ),
    ),
    node(
        ...
        steps: [
          {
            ...
            title: 'Before',
            description: wordTrunc(
              input.problem || 'Manual process, inconsistent output, high cost',
              80,
            ),
          },
          {
            id: 'with',
            title: `With ${input.companyName || 'Us'}`,
            description: wordTrunc(
              input.solution?.split('\n')[0] ||
                'Streamlined workflow, automated quality, measurable results',
              80,
            ),
          },
          {
            ...
            title: 'After',
            description: wordTrunc(
              input.traction?.split('\n')[0] || 'Faster output, higher quality, reduced overhead',
              80,
            ),
          },
        ],
    ...
    ...kpis(input)
      .slice(0, 3)
      .map((m, i) => [
        card(6 + i * 29, 60, 25, 16),
        metric(8 + i * 29, 63, 21, 8, m.value, m.label, m.delta),
      ])
      .flat(),
    bullets(6, 80, 86, 8, [
      'Repeatable across customers',
      'Consistent delivery with measurable outcomes',
      'Scalable with no added headcount',
    ]),
  ];
}
...
function designPartnership(input: WizardInput) {
  return [
    ...base(
      'Partnerships',
      `Distribution partners can turn ${input.companyName || 'us'} into a category amplifier.`,
      input.desiredAction ||
        'Partner channels strengthen acquisition, trust, and workflow embedding.',
    ),
    node(
      'featureGrid',
      { x: 6, y: 34, w: 86, h: 30 },
        ...
        items: [
          {
            id: 'channel',
            title: 'Channel',
            description: `Resellers and VARs who bring ${input.companyName || 'the product'} to existing buyer relationships`,
          },
          {
            id: 'platforms',
            title: 'Platforms',
            description: 'Integration partners who embed the product in existing workflows',
          },
          {
            id: 'strategic',
            title: 'Strategic',
            description: 'Co-sell agreements with complementary enterprise vendors',
          },
        ],
      },
      panel(),
...
function designRisks(input: WizardInput) {
  ...
  return [
    ...base(
      'Risks & Mitigations',
      'The key risks are identifiable, monitorable, and tied to mitigation plans.',
      'Investors need clarity on execution, market, product, and competitive risk.',
    ),
    node(
      'swot',
      { x: 6, y: 34, w: 58, h: 42 },
      {
        strengths: swot?.strengths || [
          `${input.companyName || 'We'} own a real, measurable customer pain`,
          'Differentiated product with proven early traction',
        ],
        weaknesses: swot?.weaknesses || [
          'Brand awareness must be built over time',
          'Enterprise sales cycle requires patience',
        ],
        opportunities: swot?.opportunities || [
          'Enterprise upsell and multi-seat expansion',
          'International markets and platform partnerships',
        ],
        threats: swot?.threats || [
          'Larger incumbents with existing distribution',
    ...
    bullets(68, 36, 24, 38, [
      'Monitor product quality and customer NPS closely',
      'Build enterprise controls early',
      'Grow customer references and case studies',
      'Maintain capital efficiency into next milestone',
    ]),
  ];
}
...
function designAppendix(input: WizardInput, slide: SlideContent) {
  return [
    ...base(
      'Appendix',
      slide.title || 'Supporting Detail',
      slide.subtitle || 'Extra investor evidence and operating detail.',
    ),
    node(
      'table',
      { x: 6, y: 34, w: 58, h: 42 },
...
function designCompetition(input: WizardInput) {
  ...
  return [
    ...base(
      'Competition',
      (() => {
        const norm = (input.differentiation || '')
          .replace(/\bvs\.\s*/gi, 'vs ')
        ...
        return wordTrunc(
          first && first.length > 6
            ? first
            : `${us} wins on design intelligence, workflow depth, and export trust`,
          78,
        );
      })(),
      conciseNarrative(
        input.differentiation,
        `${us} competes on quality, depth, and customer trust.`,
      ),
    ),
    node(
      'comparison',
      { x: 6, y: 34, w: 55, h: 45 },
      { ...c, highlightColumn: 0, accentColor: palette.accent, accentLightBg: palette.soft },
      panel(),
    ),
    card(64, 34, 28, 45),
    node(
        ...
        items: [
          {
            ...
            description: 'Presentation strategy, layout, and components are generated together.',
          },
          {
            id: 'win-2',
            title: 'Workflow depth',
            description: 'Deck creation, editing, export, and collaboration stay in one system.',
          },
          {
            id: 'win-3',
            title: 'Export trust',
            description: 'Preview and final documents are treated as the same product promise.',
          },
        ],
      },
      panel(),
    ...
    caption(
      ...
      22,
      8,
      `${us} is positioned as a complete presentation system, not a text generator.`,
    ),
  ];
}
...
function designFinancials(input: WizardInput) {
  ...
  return [
    ...base(
      'Financials',
      'A credible path from early ARR to venture-scale revenue.',
      'Forecast prioritizes gross margin, retention, and controlled expansion.',
    ),
    card(6, 34, 24, 14),
    metric(8, 37, 20, 7, f?.grossMargin || '82%', 'Gross margin'),
    card(34, 34, 24, 14),
    metric(36, 37, 20, 7, f?.burnRate || '$160K', 'Monthly burn'),
    card(62, 34, 30, 14),
    metric(64, 37, 26, 7, f?.runway || '24 mo', 'Runway post-raise'),
    node(
      'chart',
      { x: 6, y: 55, w: 54, h: 30 },
      {
        ...
        series: [
          {
            name: 'Revenue',
            values: projections.map((p) => numberish(p.revenue)),
            color: palette.accent,
          },
          {
            name: 'EBITDA',
            values: projections.map((p) => numberish(p.ebitda || '0')),
    ...
    bullets(64, 56, 28, 28, [
      'Retention-led expansion',
      'Enterprise margin profile',
      'Capital-efficient GTM',
      'Profitability scenario by year 3',
    ]),
  ];
}
...
function designAsk(input: WizardInput) {
  ...
  switch (getFamilyGroup()) {
    case 'luxury':
      return designAskLuxury(input);
    case 'startup':
      return designAskStartup(input);
    case 'consulting':
      return designAskConsulting(input);
    case 'tech':
      return designAskTech(input);
    default:
      return designAskInvestor(input);
  }
}
...
function designAskInvestor(input: WizardInput) {
  ...
  const milestoneMetric = (() => {
    ...
    for (const line of askLines) {
      ...
      for (const n of nums) {
        ...
        if (/arr|revenue|mrr/.test(ctx) && n.value !== amount)
          return { value: n.value, label: 'ARR target' };
  ...
  return [
    ...base(
      'Funding Ask',
      `${amount} to turn product proof into category leadership.`,
      conciseNarrative(
        input.fundingAsk,
        'Capital is allocated to product velocity, GTM repeatability, and enterprise readiness.',
      ),
    ),
    card(6, 34, 28, 26, palette.dark),
    metric(9, 41, 22, 9, amount, funding?.roundType || 'Round size'),
    card(38, 34, 24, 26),
    metric(41, 41, 18, 9, funding?.runway || '24 mo', 'Runway'),
    card(66, 34, 26, 26),
    metric(69, 41, 20, 9, milestoneMetric.value, milestoneMetric.label),
...
function designProduct(input: WizardInput) {
  const productTitle = wordTrunc(
    input.productService?.split(/[.!?]/)[0]?.trim() ||
      input.solution?.split(/[.!?]/)[0]?.trim() ||
      `${input.companyName || 'The platform'} — core product`,
    75,
  );
  const featureItems = [
  ...
  return [
    ...base(
      ...
      wordTrunc(
        conciseNarrative(
          input.productService || input.solution,
          `${input.companyName || 'The platform'} solves the problem with precision and reliability.`,
        ),
        120,
      ),
    ),
    node(
      ...
      {
        columns: 2,
        items: featureItems.slice(0, 6).map((f, i) => ({
          id: `pf-${i}`,
          title: wordTrunc(f.split(':')[0], 32),
          description: f.includes(':')
            ? wordTrunc(f.split(':').slice(1).join(':').trim(), 80)
      ...
      { x: 64, y: 34, w: 28, h: 40 },
      {
        steps: [
          { id: 'input', title: 'Input', description: 'Customer need or trigger' },
          {
            id: 'process',
            title: 'Process',
            description: `${input.companyName || 'Platform'} automation`,
          },
  ...
  return [
    ...
    node(
      ...
      { x: 8, y: 28, w: 76, h: 32 },
      {
        text:
          input.purpose ||
          'Every business should be able to communicate with the quality of a top-tier strategy team.',
    ...
    node(
      ...
      { x: 8, y: 68, w: 62, h: 12 },
      {
        text: 'The long-term platform opportunity is a system that transforms business intent into polished, editable, export-ready communication.',
      },
      text(14, 500, palette.textOnDark),
      'Narrative',
    ),
    card(74, 66, 18, 14, palette.soft),
    metric(76, 69, 14, 6, 'AI + Design', 'Category wedge'),
  ];
}
...
function designGeneralInvestorSlide(input: WizardInput, slide: SlideContent) {
  ...
  return [
    ...base(
      String(slide.type).replace(/_/g, ' '),
      slide.title || 'Investor Slide',
      slide.subtitle ||
        input.shortDescription ||
        'Structured for visual communication rather than plain text.',
    ),
    node(
      'featureGrid',
      { x: 6, y: 34, w: 52, h: 38 },
    ...
    caption(
      ...
      86,
      6,
      'Fallback designer guard: no generated slide is allowed to collapse into only a title and paragraph.',
    ),
  ];
}
...
export function validateSlideQA(elements: SlideElementDTO[]): SlideQAResult {
  ...
  if (coveredPct < 60) {
    issues.push({
      code: 'DEAD_SPACE',
      severity: 'error',
      detail: `Only ${Math.round(coveredPct)}% of slide height has content (threshold: 60%)`,
    });
  } else if (coveredPct < 72) {
    issues.push({
      code: 'SPARSE_LAYOUT',
      severity: 'warning',
      detail: `Slide coverage ${Math.round(coveredPct)}% — consider adding content to dead zones`,
    });
  }
  ...
  for (const el of elements) {
    if (el.x + el.width > 103) {
      issues.push({
        code: 'OVERFLOW_X',
        severity: 'error',
        detail: `${el.type} at x=${el.x.toFixed(1)} w=${el.width.toFixed(1)} overflows right edge`,
      });
    }
    if (el.y + el.height > 103) {
      issues.push({
        code: 'OVERFLOW_Y',
        severity: 'error',
        detail: `${el.type} at y=${el.y.toFixed(1)} h=${el.height.toFixed(1)} overflows bottom edge`,
      });
    }
  }
  ...
  const textNodes = elements.filter((el) =>
    ['paragraph', 'heading', 'label', 'caption'].includes(el.type),
  );
  ...
  for (const el of textNodes) {
    ...
    if (seen.has(t)) {
      issues.push({
        code: 'DUPLICATE_TEXT',
        severity: 'error',
        detail: `"${t.slice(0, 50)}" appears in both ${seen.get(t)} and ${el.type}`,
      });
    } else {
      seen.set(t, el.type);
    }
  ...
  const shapeNodes = elements.filter(
    (el) => el.type === 'shape' && (el.content as any)?.kind === 'roundedRect',
  );
  for (const shape of shapeNodes) {
    const hasContent = elements.some(
      (el) =>
        ...
        el.type !== 'shape' &&
        el.x >= shape.x - 1 &&
        el.x < shape.x + shape.width &&
        el.y >= shape.y - 1 &&
        el.y < shape.y + shape.height,
    );
    if (!hasContent) {
      issues.push({
        code: 'EMPTY_CARD',
        severity: 'warning',
        detail: `Panel at (${shape.x.toFixed(0)},${shape.y.toFixed(0)}) has no content elements inside it`,
      });
    }
  }
    ...
    if (fs > 0 && el.height > 0) {
      ...
      if (el.height < minH * 0.9) {
        issues.push({
          code: 'CLIPPED_TEXT',
          severity: 'error',
          detail: `${el.type} h=${el.height.toFixed(1)}% may clip ${fs}px text (need ≥${(minH * 1.1).toFixed(1)}%)`,
        });
      }
    }
  }
...
function designProblemLuxury(input: WizardInput) {
  ...
  const headline = (() => {
    const raw =
      input.problem?.split(/[.!?]\s+/)[0]?.trim() ||
      'A structural problem demanding an executive solution.';
    const clause = raw.split(/,\s+/)[0].trim();
    return wordTrunc(clause.length >= 15 ? clause : raw, 60);
  })();
  return [
    ...
    paragraph(
      ...
      10,
      wordTrunc(
        conciseNarrative(input.problem, 'The pain is structural, measurable, and unresolved.', 2),
        180,
      ),
      13,
    ),
    ...
    ...(secondary
      ? [
          node(
            'shape',
            { x: 65, y: 68, w: 24, h: 0.4, z: 1 },
            { kind: 'rect', fill: palette.line },
            { fill: palette.line },
            'Divider',
          ),
          node(
            ...
            { value: secondary.value, label: secondary.label },
            {
              ...headingStyle(30, palette.accent2 || palette.accent),
              letterSpacing: palette.numberLetterSpacing,
            },
          ),
        ]
      : [
          paragraph(
            ...
            24,
            14,
            conciseNarrative(input.problem, 'Validated across enterprise peer conversations.', 2),
            11,
          ),
        ]),
...
function designSolutionLuxury(input: WizardInput) {
  ...
  const capabilities = [
    ...
    wordTrunc(
      `${input.companyName || 'The platform'} delivers measurable results from day one`,
      80,
    ),
  ...
  return [
        ...
        return [
          ...
          ...(proof
            ? [
                node(
                  'metric',
                  { x: 79, y: 44.5 + i * 15, w: 11, h: 9 },
                  { value: proof.value, label: proof.label },
                  {
                    ...headingStyle(20, palette.accent),
                    letterSpacing: palette.numberLetterSpacing,
    ...
    caption(
      ...
      86,
      6,
      `${input.companyName || 'The platform'} — built for boards that demand measurable outcomes`,
    ),
  ];
}
...
function designMarketLuxury(input: WizardInput) {
  ...
  return [
    ...base(
      'Market Opportunity',
      `A ${m.tam} market with ${m.growth} annual growth.`,
    ...
    node(
      'metric',
      { x: 10, y: 34, w: 40, h: 16 },
      { value: m.tam, label: 'Total Addressable Market' },
      { ...headingStyle(54, palette.accent), letterSpacing: palette.numberLetterSpacing },
    ),
    node(
      'shape',
      { x: 54, y: 38, w: 0.5, h: 8, z: 2 },
      { kind: 'rect', fill: palette.line },
      { fill: palette.line },
      'Divider',
    ),
    node(
      'metric',
      { x: 57, y: 34, w: 16, h: 9 },
      { value: m.growth, label: 'CAGR' },
      {
        ...headingStyle(28, palette.accent2 || palette.accent),
        letterSpacing: palette.numberLetterSpacing,
      },
    ),
    node(
    ...
    node(
      'metric',
      { x: 57, y: 44, w: 32, h: 9 },
      { value: m.som, label: 'Serviceable Beachhead' },
    ...
    paragraph(
      ...
      wordTrunc(
        conciseNarrative(
          input.marketOpportunity || input.solution || '',
          `${input.industry || 'Enterprise'} demand is compounding — driven by regulatory pressure, workflow complexity, and the shift to AI-native operations.`,
        ),
        280,
      ),
      11,
    ...
    node(
      ...
      {
        columns: 1,
        items: marketSizing(input)
          .drivers.slice(0, 3)
          .map((d, i) => ({
            id: `d-${i}`,
            title: `Driver ${i + 1}`,
            description: wordTrunc(d, 70),
          })),
      },
      panel(),
    ),
    caption(
      ...
      86,
      6,
      'Source: primary research across enterprise CFOs and procurement leaders',
    ),
  ];
}
...
function designTeamLuxury(input: WizardInput) {
  ...
  const credBar = [
    ...
    input.traction?.match(/\$[\d.]+[KMB]/)?.[0]
      ? `${input.traction.match(/\$[\d.]+[KMB]/)?.[0]} ARR built`
      : null,
  ...
  return [
    ...base(
      'Leadership',
      `${input.companyName || 'Company'} leadership: ${members.map((m) => m.role).join(', ')}.`,
    ...
    ...members
      .map((m, i) => {
        ...
        return [
          ...
          node(
            'label',
            { x: 9 + i * 29.5, y: 36, w: 21, h: 4 },
            { text: m.name },
            { ...headingStyle(16, palette.ink), fontWeight: 700 },
          ),
          node(
            'label',
            { x: 9 + i * 29.5, y: 41, w: 21, h: 3 },
            { text: m.role.toUpperCase() },
            { ...label(9), color: palette.accent },
          ),
          node(
            'shape',
            { x: 9 + i * 29.5, y: 45, w: 18, h: 0.3, z: 1 },
            { kind: 'rect', fill: palette.line },
            { fill: palette.line },
            'Bio rule',
          ),
          ...
          ...(badge
            ? [
                node(
                  'label',
                  { x: 9 + i * 29.5, y: 62, w: 21, h: 5 },
                  { text: badge },
                  { ...headingStyle(14, palette.accent), fontStyle: 'italic' as any },
                ),
              ]
            : []),
        ];
    ...
    caption(
      ...
      86,
      6,
      'Investor lens: institutional pedigree with operating-level accountability',
    ),
  ];
}
...
function designAskLuxury(input: WizardInput) {
  ...
  return [
    ...base(
      'Investment',
      `${amount} to establish category leadership.`,
      conciseNarrative(
        input.fundingAsk,
        'Capital deployed across product velocity, enterprise GTM, and operational resilience.',
      ),
    ),
    ...
    node(
      'metric',
      { x: 10, y: 34, w: 32, h: 14 },
      { value: amount, label: 'Investment ask' },
      { ...headingStyle(54, palette.accent), letterSpacing: palette.numberLetterSpacing },
    ),
    node(
      'shape',
      { x: 46, y: 36, w: 0.5, h: 10, z: 2 },
      { kind: 'rect', fill: palette.line },
      { fill: palette.line },
      'Divider',
    ),
    ...
    node(
      'metric',
      { x: 71, y: 34, w: 18, h: 14 },
      { value: milestoneMetric.value, label: milestoneMetric.label },
    ...
    ...allocations
      .map((a, i) => [
        node(
          'shape',
          { x: 6, y: 59 + i * 10, w: ((a.percentage || 0) / 100) * 76, h: 6, z: 1 },
          {
            kind: 'rect',
            fill:
              i === 0
          ...
          { fill: palette.accent, borderRadius: 4 },
          'Alloc bar',
        ),
        node(
          'label',
          { x: 84, y: 59 + i * 10, w: 8, h: 6 },
          { text: `${a.percentage}%` },
          { ...label(10), textAlign: 'right' as any, color: palette.ink },
        ),
        node(
          'label',
          { x: 6, y: 59 + i * 10, w: 40, h: 6 },
          { text: a.category },
    ...
    caption(
      ...
      86,
      6,
      `${input.companyName || 'We'} — building the category-defining platform in ${input.industry || 'this space'}`,
    ),
  ];
}
...
function designProblemStartup(input: WizardInput) {
  ...
  const narrative = wordTrunc(
    sents.slice(1, 3).join('. ') ||
      conciseNarrative(input.problem, 'Every team we talked to felt it.', 2),
    160,
  );
  const consequences = [
    ...lines(input.problem || ''),
    ...
    ...(secondary
      ? [
          node(
            ...
            { value: secondary.value, label: secondary.label },
            {
              ...headingStyle(16, palette.accent2 || palette.accent),
              letterSpacing: palette.numberLetterSpacing,
            },
          ),
        ]
...
function designSolutionStartup(input: WizardInput) {
  ...
  const slideTitle = wordTrunc(rawTitle.split(/,\s+/)[0].trim() || rawTitle, 60);
  const beforeText = wordTrunc(
    conciseNarrative(input.problem, 'Manual, inconsistent, error-prone.', 1),
    120,
  );
  const afterText = (() => {
    const kpiData = kpis(input);
    if (kpiData.length >= 2)
      return `${kpiData[0].value} ${kpiData[0].label} · ${kpiData[1].value} ${kpiData[1].label} · proven at scale`;
    return wordTrunc(
      conciseNarrative(input.solution, 'Fast, accurate, and enterprise-ready.', 1),
      120,
    );
  })();
...
function designMarketStartup(input: WizardInput) {
  ...
  return [
    ...base(
      'Market',
      `We're riding the ${input.industry || 'automation'} wave.`,
    ...
    node(
      'label',
      { x: 6, y: 31, w: 86, h: 4 },
      { text: `THE ${(input.industry || 'AUTOMATION').toUpperCase()} MARKET IS INFLECTING NOW` },
      { ...label(8), color: palette.accent },
    ),
    ...
    ...[
      ...
      return [
        ...
        node(
          'shape',
          { x: 6, y: 43 + i * 12, w: barWidth, h: 5, z: 1 },
          {
            kind: 'rect',
            fill:
              i === 0
          ...
          { fill: palette.accent, borderRadius: 4, opacity: i === 0 ? 1 : 0.7 },
          'Market bar',
        ),
        node(
          'label',
          { x: valueX, y: 43 + i * 12, w: 10, h: 5 },
          { text: item.value },
    ...
    node(
      ...
      {
        columns: 3,
        items: m.drivers.slice(0, 3).map((d, i) => ({
          id: `td-${i}`,
          title: `Tailwind ${i + 1}`,
          description: wordTrunc(d, 60),
        })),
      },
      panel(),
    ),
...
function designTeamStartup(input: WizardInput) {
  ...
  const whyWeWinItems = [
    {
      ...
      description: (() => {
        ...
        if (yr)
          return wordTrunc(
            `${yr[0]} of hands-on industry experience${prev ? ` (ex-${prev[1]})` : ''}`,
            80,
          );
        return wordTrunc(
          input.industry
            ? `Deep expertise in ${input.industry} with operational domain knowledge`
            : 'Founders with direct domain expertise in the problem space',
          80,
        );
      })(),
    },
    {
      ...
      description: (() => {
        ...
        if (arrM && custM)
          return wordTrunc(`${arrM[0]}, ${custM[1]} customers — revenue before fundraising`, 80);
        return wordTrunc(
          conciseNarrative(
            input.traction,
            'Proven execution: revenue and customers before fundraising',
          ),
          80,
        );
      })(),
    },
    {
      ...
      description: (() => {
        ...
        return wordTrunc(
          conciseNarrative(
            input.differentiation,
            'Structural advantage competitors cannot easily replicate',
          ),
          80,
        );
      })(),
  ...
  return [
    ...base(
      'Team',
      `${input.companyName || 'We'} — built by people who lived the problem.`,
    ...
    ...members
      .map((m, i) => [
        ...
        node(
          'label',
          { x: 9 + i * 29.5, y: 35, w: 21, h: 5 },
          { text: m.name },
          { ...headingStyle(18, palette.ink), fontWeight: 800 },
        ),
        node(
          'label',
          { x: 9 + i * 29.5, y: 41, w: 21, h: 4 },
          { text: m.role },
          { ...label(9), color: palette.accent },
        ),
...
function designAskStartup(input: WizardInput) {
  ...
  const unlocks = [
    {
      title: 'Product',
      unlock: allocations[0]
        ? `${allocations[0].percentage}% — ${allocations[0].category}`
        : 'Ship 3 enterprise features',
      metric: `${runway} runway`,
    },
    {
      title: 'Growth',
      unlock: allocations[1]
        ? `${allocations[1].percentage}% — ${allocations[1].category}`
        : 'Hire 4 AEs + 2 SDRs',
      metric: '3× pipeline',
    },
    {
      title: 'Operations',
      unlock: allocations[2]
        ? `${allocations[2].percentage}% — ${allocations[2].category}`
        : 'SOC 2 + compliance',
  ...
  const milestoneMetric = (() => {
    ...
    return arrKpi
      ? { value: '3×', label: `${arrKpi.label} milestone` }
      : { value: 'Series B', label: 'Next raise' };
  })();

  return [
    ...base(
      'Raise',
      `${amount} — to go from product proof to category leader.`,
    ...
    node(
      'metric',
      { x: 10, y: 36, w: 32, h: 16 },
      { value: amount, label: funding?.roundType || 'Round' },
      { ...headingStyle(54, palette.accent), letterSpacing: palette.numberLetterSpacing },
    ),
    node(
      'metric',
      ...
      { value: runway, label: 'Runway' },
      {
        ...headingStyle(18, palette.accent2 || palette.accent),
        letterSpacing: palette.numberLetterSpacing,
      },
    ),
    node(
    ...
    node(
      'label',
      { x: 6, y: 65, w: 86, h: 4 },
      { text: 'ARR GROWTH MILESTONES' },
      { ...label(8), color: palette.accent },
    ),
    ...['Today', '+12 mo', '+24 mo', '+30 mo']
      .map((period, i) => [
        card(6 + i * 21.5, 70, 20, 18, i === 3 ? palette.dark : palette.panel),
        node(
          'label',
          { x: 9 + i * 21.5, y: 72, w: 14, h: 3 },
          { text: period },
          { ...label(8), color: i === 3 ? palette.accent : palette.muted },
        ),
        node(
          'metric',
          { x: 9 + i * 21.5, y: 76, w: 14, h: 10 },
          { value: ['$1.8M', '$4M', '$8M', '$15M'][i], label: 'ARR' },
          {
            ...headingStyle(20, i === 3 ? palette.accent : palette.ink),
            letterSpacing: palette.numberLetterSpacing,
          },
        ),
      ])
...
function designProblemConsulting(input: WizardInput) {
  ...
  const headline = (() => {
    const r =
      input.problem?.split(/[.!?]\s+/)[0]?.trim() ||
      'Operational inefficiency at enterprise scale.';
    return wordTrunc(r.split(/,\s+/)[0].trim() || r, 60);
  })();
  ...
  return [
    ...
    ...(secondary
      ...
      : [
          paragraph(
            ...
            conciseNarrative(
              input.problem,
              'Benchmark against industry peers confirms the gap.',
              2,
            ),
            11,
          ),
    ...
    paragraph(
      ...
      78,
      3,
      `Target efficiency gain: ${hero.value} reduction in ${hero.label.toLowerCase()} through systematic automation`,
      10,
    ),
  ];
}
...
function designSolutionConsulting(input: WizardInput) {
  ...
  const capItems = [
    ...solLines,
    wordTrunc(
      `${input.companyName || 'The platform'} captures and structures all input with precision`,
      80,
    ),
  ...
  return [
    ...
    ...columns
      .map((col, i) => [
        ...
        node(
          'label',
          { x: 9 + i * 29.5, y: 40, w: 21, h: 4 },
          { text: `${i + 1}. ${col.toUpperCase()}` },
          { ...label(9), color: palette.accent },
        ),
    ...
    paragraph(
      ...
      78,
      3,
      'Zero-code deployment · Day-1 ROI · Phased rollout with measurable milestones per quarter',
      10,
    ),
  ];
}
...
function designMarketConsulting(input: WizardInput) {
  ...
  return [
    ...
    ...[
      ...
      node(
        'label',
        { x: 6, y: 37 + i * 14, w: 12, h: 4 },
        { text: row.label },
        { ...label(9), color: palette.accent },
      ),
      node(
        'metric',
        { x: 19, y: 36 + i * 14, w: 26, h: 8 },
        { value: row.value, label: row.desc },
        { ...headingStyle(24, palette.ink), letterSpacing: palette.numberLetterSpacing },
      ),
      node(
        'shape',
        { x: 6, y: 47 + i * 14, w: 48, h: 0.3, z: 1 },
        { kind: 'rect', fill: palette.line },
        { fill: palette.line },
        'Row line',
      ),
    ...
    node(
      'metric',
      { x: 6, y: 76, w: 22, h: 10 },
      { value: m.growth, label: 'Market CAGR' },
...
function designTeamConsulting(input: WizardInput) {
  ...
  return [
    ...base(
      'Team',
      `${input.companyName || 'Company'} — domain-led, execution-proven.`,
    ...
    ...members
      .map((m, i) => [
        node(
          'shape',
          { x: 6, y: 38 + i * 14, w: 86, h: 12, z: -1 },
          {
            kind: 'roundedRect',
            fill: i % 2 === 0 ? palette.soft : palette.panel,
            stroke: palette.line,
            strokeWidth: 0.5,
          },
          panel(i % 2 === 0 ? palette.soft : palette.panel),
          'Row bg',
        ),
        paragraph(9, 39.5 + i * 14, 23, 9, domains[i] || m.role, 10),
        node(
          'label',
          { x: 34, y: 39.5 + i * 14, w: 28, h: 5 },
          { text: m.name },
          { ...headingStyle(13, palette.ink), fontWeight: 700 },
        ),
        node(
          'label',
          { x: 34, y: 45 + i * 14, w: 28, h: 4 },
          { text: m.role },
          { ...label(9), color: palette.accent },
        ),
    ...
    paragraph(
      ...
      78,
      4,
      `${members.length} functional leads · zero key-person risk · ${input.industry || 'domain'} operator experience at every seat`,
      10,
    ),
...
function designAskConsulting(input: WizardInput) {
  ...
  const milestones = [
    {
      quarter: 'Q1–Q2',
      outcome: 'Enterprise feature parity + SOC 2 certification',
      gate: '200% ARR growth',
    },
    {
      quarter: 'Q3',
      outcome: 'Channel partnerships + expansion into adjacent verticals',
      gate: 'Series B pipeline',
    },
    {
      quarter: 'Q4',
      outcome: 'International expansion readiness + platform API launch',
      gate: 'Series B raise',
    },
  ];
  ...
  return [
    ...base(
      'Investment Case',
      `${amount} — strategic capital allocation.`,
      conciseNarrative(
        input.fundingAsk,
        'Capital deployed against three measurable growth vectors with defined exit gates.',
      ),
    ),
    // Header row
    node(
    ...
    ...allocations
      .map((a, i) => {
        ...
        const useOfFundsMap: Record<string, string> = {
          product: wordTrunc(
            ensure(lines(input.roadmap || ''), ['Enterprise features + API platform'], 1)[0],
            40,
          ),
          engineering: wordTrunc(
            ensure(lines(input.roadmap || ''), ['Core build + infrastructure scaling'], 1)[0],
            40,
          ),
        ...
        return [
          ...
          paragraph(
            ...
            24,
            5,
            useOfFundsMap[catKey] || 'Milestone-gated deployment',
            10,
          ),
          paragraph(
            62,
            ...
            28,
            5,
            outcomeMap[catKey] || 'Measurable benchmark improvement',
            10,
          ),
        ];
      })
    ...
    // Key metrics
    card(6, 75, 20, 12),
    metric(8, 77, 16, 8, amount, 'Investment'),
    card(29, 75, 18, 12),
    metric(31, 77, 14, 8, runway, 'Runway'),
    card(50, 75, 18, 12),
    metric(
      52,
      77,
...
function designProblemTech(input: WizardInput) {
  ...
  const stages = [
    ...
    .slice(0, 3);
  const headline = (() => {
    const r = input.problem?.split(/[.!?]\s+/)[0]?.trim() || 'The bottleneck is architectural.';
    return wordTrunc(r.split(/,\s+/)[0].trim() || r, 60);
  })();
  ...
  return [
    ...
    paragraph(
      ...
      wordTrunc(
        conciseNarrative(
          input.problem,
          'The bottleneck creates compounding latency across all downstream systems.',
          2,
        ),
        160,
      ),
    ...
    ...(secondary
      ? [
          node(
            ...
            { value: secondary.value, label: secondary.label },
            {
              ...headingStyle(22, palette.accent2 || palette.accent),
              letterSpacing: palette.numberLetterSpacing,
            },
          ),
        ]
    ...
    ...kpis(input)
      ...
      .map((m, i) => [
        ...
        node(
          'label',
          { x: 9 + i * 21, y: 75, w: 13, h: 3.5 },
          { text: wordTrunc(m.label, 15) },
          { ...label(7), color: palette.muted },
        ),
        node(
          'label',
          { x: 9 + i * 21, y: 79, w: 13, h: 4 },
          { text: m.value },
          { ...headingStyle(14, palette.accent) },
        ),
      ])
...
function designSolutionTech(input: WizardInput) {
  ...
  const slideTitle = wordTrunc(rawTitle.split(/,\s+/)[0].trim() || rawTitle, 60);
  const components = [
    ...lines(input.solution || ''),
    ...[
      wordTrunc(
        `${input.companyName || 'Core'} layer: AI-powered intelligence with high precision`,
        80,
      ),
  ...
  return [
    ...
    ...[
      {
        label: 'INGEST',
        sub: wordTrunc(lines(input.solution)[0] || 'Data entry and ingestion', 22),
      },
      {
        label: 'PROCESS',
        sub: wordTrunc(lines(input.solution)[1] || 'Automated processing layer', 22),
      },
      {
        label: 'VALIDATE',
        sub: wordTrunc(lines(input.solution)[2] || 'Quality validation and review', 22),
      },
      {
        label: 'DELIVER',
        sub: wordTrunc(conciseNarrative(input.traction, 'Outputs delivered to users', 1), 22),
      },
    ]
      .map((stage, i) => [
        node(
          'label',
          { x: 10 + i * 21, y: 37, w: 17, h: 4 },
          { text: stage.label },
          { ...label(9), color: palette.accent },
        ),
        node(
          'label',
          { x: 10 + i * 21, y: 41, w: 17, h: 4 },
          { text: stage.sub },
          text(9, 400, palette.muted),
        ),
        ...(i < 3
          ? [
              node(
                'label',
                { x: 26.5 + i * 21, y: 38, w: 3, h: 8 },
                { text: '→' },
    ...
    ...components
      .map((comp, i) => [
        ...
        node(
          'shape',
          { x: 6 + i * 29.5, y: 49, w: 27, h: 0.6, z: 3 },
          { kind: 'rect', fill: palette.accent },
          { fill: palette.accent },
          'Comp rule',
        ),
    ...
    ...perfMetrics
      ...
      .map((m, i) => [
        ...
        node(
          'label',
          { x: 9 + i * 21, y: 83, w: 13, h: 3.5 },
          { text: m.label },
          { ...label(7), color: palette.muted },
        ),
        node(
          'label',
          { x: 9 + i * 21, y: 87, w: 13, h: 4 },
          { text: m.value },
          { ...headingStyle(14, palette.accent) },
        ),
      ])
...
function designMarketTech(input: WizardInput) {
  ...
  return [
    ...base(
      'Market',
      `${m.tam} market — entering the steep adoption curve.`,
    ...
    node(
      'metric',
      { x: 67, y: 33, w: 22, h: 12 },
      { value: m.tam, label: 'TAM' },
      { ...headingStyle(34, palette.accent), letterSpacing: palette.numberLetterSpacing },
    ),
    card(64, 50, 14, 10),
    metric(66, 52, 10, 6, m.sam, 'SAM'),
    card(80, 50, 12, 10),
    metric(82, 52, 8, 6, m.growth, 'CAGR'),
    // Developer/tech adoption triggers
    node(
      'label',
      { x: 6, y: 70, w: 86, h: 3 },
      { text: 'CATEGORY ADOPTION TRIGGERS' },
      { ...label(8), color: palette.accent },
    ),
    node(
      'featureGrid',
    ...
    caption(
      ...
      86,
      6,
      `${input.companyName || 'Platform'} is positioned in the inflection zone of the adoption curve`,
    ),
  ];
}
...
function designTeamTech(input: WizardInput) {
  ...
  return [
    ...base(
      'Team',
      `${input.companyName || 'Company'} — builders with production-grade track records.`,
    ...
    ...members
      .map((m, i) => [
        ...
        node(
          'shape',
          { x: 6 + i * 29.5, y: 32, w: 27, h: 0.6, z: 3 },
          { kind: 'rect', fill: palette.accent },
          { fill: palette.accent },
          'Card rule',
        ),
        node(
          'label',
          { x: 9 + i * 29.5, y: 34, w: 21, h: 4.5 },
          { text: m.name },
          { ...headingStyle(15, palette.ink), fontWeight: 800 },
        ),
        node(
          'label',
          { x: 9 + i * 29.5, y: 39, w: 21, h: 3.5 },
          { text: m.role.toUpperCase() },
          { ...label(8), color: palette.accent },
        ),
...
function designAskTech(input: WizardInput) {
  ...
  const milestones =
    roadmapPhases.length >= 3
      ? roadmapPhases.slice(0, 3).map((r, i) => ({
      ...
      : [
          {
            phase: 'Phase 1',
            desc: wordTrunc(
              `${input.companyName || 'Platform'}: core product + enterprise controls`,
              55,
            ),
            gate: '100 customers',
          },
          {
            phase: 'Phase 2',
            desc: wordTrunc(`API platform + integrations + go-to-market expansion`, 55),
            gate: '250 customers',
          },
          {
            phase: 'Phase 3',
            desc: wordTrunc(`International expansion + Series B milestones`, 55),
            gate: 'Series B',
          },
        ];
  ...
  return [
    ...base(
      'Raise',
      `${amount} — engineering the next product generation.`,
      conciseNarrative(
        input.fundingAsk,
        'Every dollar is mapped to a shipped milestone, not a burn runway.',
      ),
    ),
    ...
    ...milestones
      .map((ms, i) => [
        ...
        node(
          'shape',
          { x: 6, y: 36 + i * 14, w: 50, h: 0.6, z: 3 },
          { kind: 'rect', fill: i === 0 ? palette.accent : palette.line },
          { fill: palette.accent },
          'Mile rule',
        ),
        node(
          'label',
          { x: 9, y: 38 + i * 14, w: 18, h: 3.5 },
          { text: ms.phase },
          { ...label(8), color: palette.accent },
        ),
        node(
          'label',
          { x: 28, y: 38 + i * 14, w: 22, h: 3.5 },
          { text: `GATE: ${ms.gate}` },
          { ...label(7), color: palette.muted },
        ),
    ...
    ...allocations
      .map((a, i) => [
        ...
        node(
          'label',
          { x: 63, y: 69.5 + i * 7, w: 24, h: 4 },
          { text: `${a.category}: ${a.percentage}%` },
          text(10, 500, palette.ink),
        ),
      ])
      .flat(),
