...
export class CvExportQueueProcessor {
  ...
  async handleExport(job: Job<CvExportJobData>): Promise<CvExportJobResult> {
    ...
    const t0 = Date.now();
    this.logger.log(`[EXPORT-QUEUE] start job=${job.id} doc=${documentId} fmt=${format} user=${userId}`);
    ...
    this.telemetry.track('export_done', { userId, durationMs, meta: { fmt: format, docId: documentId, mode: r.mode, queued: true } });
