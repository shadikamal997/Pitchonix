...
export class CvSnapshotService {
  ...
  async list(
    userId: string,
    opts: { profileId?: string; documentId?: string; kind?: CvSnapshotKind; limit?: number } = {},
  ) {
...
export class CvVariantsService {
  ...
  private pickTemplate(
    ...
    const candidates = templates.filter((t) => {
      ...
      return preset.templateCategoryPreference.some((p) =>
        (t.category || '').toLowerCase().includes(p),
      );
    });
    ...
    for (const preset of opts.presets) {
      ...
      try {
        await this.documents.update(doc.id, {
          content: {
            ...(doc.content || {}),
            sectionOrder: spec.sectionOrder as any,
            variantPreset: preset,
          } as any,
        });
      } catch {
        /* non-fatal — empty section order falls back to default */
      }
...
const BAND = (
  ...
  industry: number,
  top10: number,
): { percentile: number; band: BenchmarkBand['band'] } => {
...
export class CvBenchmarkService {
  benchmark(report: CvQualityReport): CvBenchmarkReport {
    ...
    const notes: string[] = [];
    if (atsBand.band === 'top10')
      notes.push(
        'Your ATS score is in the top decile — strong choice of template + clean content.',
      );
    if (measBand.band === 'belowAvg' || measBand.band === 'bottom')
      notes.push(
        'Adding measurable results to more bullets is the single highest-impact improvement available.',
      );
    if (pageEff < 50)
      notes.push('Your CV runs long; recruiters skip past page 2 for most non-executive roles.');
    ...
    return {
      ...
      user: {
        ats: report.atsScore,
        impact: report.subscores.impact,
        readability: report.subscores.readability,
      },
      bands: {
        ats: {
          metric: 'ATS score',
          ...
          unit: 'score',
          ...atsBand,
          hint: 'Cleaner templates + plain text bullets + present contact info.',
        },
        impact: {
          metric: 'Impact',
          value: report.subscores.impact,
          industry: 65,
          top10: 88,
          unit: 'score',
          ...impactBand,
          hint: 'Action verbs + measurable outcomes.',
        },
        readability: {
          metric: 'Readability',
          value: report.subscores.readability,
          industry: 70,
          top10: 90,
          unit: 'score',
          ...readBand,
          hint: 'Trim long bullets, prefer active voice.',
        },
        pageEfficiency: {
          metric: 'Page efficiency',
          value: pageEff,
          industry: 65,
          top10: 90,
          unit: 'score',
          ...pageBand,
          hint: 'Target 1–2 pages unless executive / academic.',
        },
        actionVerbs: {
          metric: 'Action-verb ratio',
          value: m.actionVerbRatio,
          industry: 55,
          top10: 80,
          unit: 'percent',
          ...verbBand,
          hint: 'Start bullets with verbs like Led / Shipped / Built.',
        },
        measurables: {
          metric: 'Measurable bullets',
          value: m.measurableRatio,
          industry: 35,
          top10: 70,
          unit: 'percent',
          ...measBand,
          hint: 'Add a number to every other bullet (%, $, users, time).',
        },
        skillsCoverage: {
          metric: 'Skills coverage',
          value: Math.min(100, skillsCount),
          industry: 50,
          top10: 90,
          unit: 'score',
          ...skillsBand,
          hint: 'List 8–20 concrete skills; ATS systems score on this.',
        },
      },
      notes,
    };
...
export class CvInterviewReadinessService {
  analyze(profile: CvProfileSnapshot): InterviewReadinessReport {
    ...
    const leaderClaim = /\b(led|managed|owned|directed|head of|hired|coached|mentored)\b/i.test(
      bulletText,
    );
    if (leaderClaim) {
      hiringMgrQ.push({
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      const a = parseYM(prevEnd);
      const b = parseYM(curStart);
      if (!a || !b) continue;
    ...
    if (personal.fullName) {
      recruiterQ.push({
        q: 'Walk me through your CV in 2 minutes.',
        rationale: 'Universal opener — practice a 90-second pitch.',
      });
      recruiterQ.push({
        q: 'What are you looking for in your next role?',
        rationale: 'Filter for fit + motivation.',
      });
      recruiterQ.push({
        q: 'What is your notice period / earliest start date?',
        rationale: 'Logistics check.',
      });
    ...
    if (missingMeasurables > bullets.length * 0.7) {
      weakAreas.push(
        'Few bullets contain measurable results — expect "What was the impact?" follow-ups.',
      );
    }
    if (claimedNotDemo.length > 3)
      weakAreas.push(`${claimedNotDemo.length} listed skills have no project / bullet evidence.`);
    if (gaps.length > 0)
      weakAreas.push(`${gaps.length} chronology gap(s) over 6 months — prepare a short narrative.`);
    if (!personal.summary)
      weakAreas.push(
        'No professional summary — recruiters often paraphrase it back to you in the first call.',
      );
...
export class CvTemplateInsightsService {
  ...
  scoreOne(t: {
    ...
    const balanced = Math.round(
      (axes.ats + axes.visual + axes.executive + axes.creative + axes.readability + axes.print) / 6,
    );
    const bestFor = (Object.keys(axes) as Array<keyof TemplateInsightAxes>).reduce(
      (best, k) => (axes[k] > axes[best] ? k : best),
      'balanced' as any,
    );
  ...
  scoreMany(
    rows: Array<{ id: string; name: string; category?: string | null; layout: any }>,
  ): TemplateInsight[] {
