...
const SECTION_HEADING_SET = new Set([
  ...
  'internship', 'internships', 'volunteering', 'volunteer experience',
  // Education
  'education', 'academic background', 'educational background', 'qualifications',
  'academic qualifications', 'academic', 'academic history', 'training', 'courses',
  // Skills
  'skills', 'technical skills', 'core skills', 'key skills', 'competencies',
  'core competencies', 'technical stack', 'tech stack', 'tools', 'expertise',
  ...
  'get in touch', 'find me',
  // Projects / Certs / Awards
  'projects', 'side projects', 'portfolio', 'selected projects',
  'certifications', 'certificates', 'licenses & certifications',
  'awards', 'awards & honors', 'achievements', 'honors', 'recognition',
...
const SECTION_LABEL_RE = /^(key\s+skills|skills\s*:|contact\s*:|profile\s*:|summary\s*:|education\s*:|experience\s*:|reference\s*:)/i;
...
const EDUCATION_POISON_RE = /\b(javascript|typescript|python|react|angular|vue|node\.?js|css|html|sql|java|php|ruby|swift|kotlin|docker|kubernetes|git|agile|scrum|redux|graphql|rest\s*api|aws|azure|gcp|linux|django|laravel|spring|next\.?js|tailwind|bootstrap|mongodb|postgresql|mysql)\b/i;
const EXPERIENCE_VERB_RE  = /^(collaborated|developed|managed|worked|designed|implemented|coordinated|created|maintained|led|built|achieved|established|provided|ensured|delivered|prepared|analysed|analyzed|supported|handled|conducted|performed|executed|produced|completed|monitored|supervised|trained|mentored|organized|researched|resolved|processed|deployed|integrated|automated|optimized|generated|troubleshot|reviewed|negotiated|launched|increased|reduced|facilitated)\b/i;
...
export function sanitizeCvProfile(
  profile: CvProfileDto,
): { profile: CvProfileDto; report: SanitizeReport } {
  const report: SanitizeReport = {
    fullNameFixed: false, headlineFixed: false, summaryFixed: false,
    experienceFixed: 0, educationFixed: 0, skillsFixed: 0, anyChange: false,
  };

  // ── Personal ──────────────────────────────────────────────────────────
  let personal = { ...(profile.personal || {}) } as any;
  ...
  const experience = (profile.experience || []).filter((exp: any) => {
    ...
    if (isSectionHeading(role)) return false;
    // Keep entries with at least a role or company.
    if (!role && !(exp.company || '').trim()) return false;
    return true;
  });
  ...
  const education = (profile.education || []).filter((edu: any) => {
    ...
    if (isSectionHeading(inst) || isSectionHeading(degree) || isSectionHeading(field)) return false;
    if (EDUCATION_POISON_RE.test(blob)) return false;
    if (EXPERIENCE_VERB_RE.test(blob)) return false;
    if (/\b(tour operator|frontend|front-end|developer|designer|collaborated with designers|freelancer|current)\b/i.test(blob)) return false;
    // Reject bare date strings as institution names (OCR artifact).
    if (/^\d{4}([-–—]\d{4})?$/.test(inst)) return false;
    return true;
  });
  ...
  const skills = (profile.skills || []).filter((sk: any) => {
    ...
    if (isSectionHeading(name)) return false;
    // Reject if it reads like a full sentence (action verb + object).
    if (SKILL_SENTENCE_RE.test(name) && name.split(/\s+/).length > 6) return false;
    return true;
  });
  ...
  // ── Assemble result ──────────────────────────────────────────────────
  report.anyChange =
    report.fullNameFixed || report.headlineFixed || report.summaryFixed ||
    report.experienceFixed > 0 || report.educationFixed > 0 || report.skillsFixed > 0;
...
export function sanitizeCvSummaryText(text: string): string {
  ...
  const kept = lines.filter((line) => {
    const t = line.trim().replace(/\s*[,;:]?\s*[\{\[\(]\s*[,;:]?\s*$/g, '').trim();
    if (!t) return false;
