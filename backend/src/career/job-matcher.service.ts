...
export interface JobGapFix {
  action: 'add-skill' | 'add-keyword' | 'improve-experience' | 'add-certification' | 'highlight-experience';
  payload: any;
  autoApply: boolean;
}
...
export class JobMatcherService {
  ...
  private parseJobDescription(jobDescription: string): any {
    ...
    const certKeywords = ['aws certified', 'pmp', 'scrum master', 'cissp', 'comptia', 'microsoft certified'];
  ...
  private matchEducation(profile: any, jobReqs: any): MatchCategory {
    ...
    if (requiredEducation) {
      if (education.length === 0) {
        ...
        const hasDegree = education.some((edu: any) => 
          edu.degree && /bachelor|master|phd|degree/i.test(edu.degree)
        );
    ...
    return {
      ...
      details: education.length > 0 
        ? `${education.length} education entry(ies)` 
        : 'No education listed'
    };
  }
  ...
  private matchCertifications(profile: any, jobReqs: any): MatchCategory {
    ...
    const score = requiredCerts.length > 0
      ? Math.round((matched.length / requiredCerts.length) * 100)
      : 100;
    ...
    return {
      ...
      matched,
      missing,
      details: requiredCerts.length > 0
  ...
  private generateImprovements(gaps: JobGap[], breakdown: any): JobImprovement[] {
    ...
    gaps.filter(g => g.severity === 'important').slice(0, 5).forEach(gap => {
      improvements.push({
        id: `imp-${id++}`,
        priority: 'medium',
        title: gap.type === 'keyword' 
          ? `Include keyword: ${gap.item}`
          : `Address: ${gap.item}`,
        description: gap.description,
    ...
    gaps.filter(g => g.severity === 'nice-to-have').slice(0, 3).forEach(gap => {
      improvements.push({
        id: `imp-${id++}`,
        priority: 'low',
   ...
   */
  private getRecommendation(score: number): 'strong-match' | 'good-match' | 'partial-match' | 'weak-match' {
