...
export interface AnalyzeSlideContentRequest {
  content: string;
  slideType?: string; // problem, solution, market, team, etc.
  dataPoints?: {
    label: string;
    value: number;
  }[];
  context?: {
    companyName?: string;
    industry?: string;
    designStyle?: string; // modern, minimal, bold, professional
  };
}
...
export interface ChartRecommendation {
  ...
  reason: string;
  example: string; // Description of what chart would show
  pros: string[];
  cons: string[];
}
...
export interface LayoutRecommendation {
  ...
  structure: {
    sections: string[];
    emphasis: string; // What should be emphasized
  };
  preview?: string; // ASCII art or description
}

export interface ImageRecommendation {
  style: ImageStyle;
  confidence: number;
  searchKeywords: string[]; // For finding appropriate images
...
export interface GenerateChartDataRequest {
  description: string; // What the chart should show
  chartType: ChartType;
  dataPoints?: number; // How many data points (default: 5)
}
...
export interface ChartDataResponse {
  ...
  data: {
    labels: string[];
    values: number[];
    categories?: string[]; // For multi-series charts
  };
  title: string;
  subtitle?: string;
  formatting: {
