...
export class MagazineLayoutEngineService {
  ...
  ): { page: PageComposition; magazine: MagazineCompositionPlan } {
  ...
  private planMagazineComposition(
    page: PageComposition,
    grid: EditorialGridPlan,
  ): MagazineCompositionPlan {
    ...
    const quote =
      page.sections.find((section) => section.type === 'quote')?.content ||
      this.extractPullQuote(text);
    ...
    const hasAnalytics =
      !!metric ||
      page.sections.some((section) => section.type === 'metric' || section.type === 'chart');
    ...
    else if (grid.gridType === 'asymmetric' || grid.gridType === 'sidebar')
      archetype = 'editorial-article';
    else if (grid.gridType === 'image-heavy') archetype = 'feature-callout';

    return {
      archetype,
      visualTone: hasAnalytics
        ? 'data-rich'
        : archetype === 'cover' || archetype === 'quote-spread'
          ? 'cinematic'
          : grid.columns > 1
            ? 'editorial'
            : 'structured',
  ...
  private applyMagazineDirection(
    page: PageComposition,
    ...
    return {
      ...
      sections: page.sections.map((section, index) => {
        if (section.type === 'heading') {
          return {
            ...
            visualWeight: Math.max(
              section.visualWeight,
              magazine.archetype === 'editorial-article' ? 92 : 86,
            ),
            fontSize:
              magazine.archetype === 'cover'
                ? 2.8
                : magazine.archetype === 'quote-spread'
                  ? 2.15
                  : 1.95,
            lineHeight: 1.08,
        ...
        if (
          ...
          index === 1 &&
          magazine.archetype === 'editorial-article'
        ) {
          return {
            ...section,
            visualWeight: 70,
      ...
      metrics: {
        ...page.metrics,
        visualBalanceScore: Math.round(
          (page.metrics.visualBalanceScore + magazine.qualityScore) / 2,
        ),
  ...
  private extractMetric(text: string): string | undefined {
    return text.match(
      /\b\d+(?:\.\d+)?\s?(?:%|x|k|m|b|million|billion|users|customers|revenue)\b/i,
    )?.[0];
  }
  ...
  private pickSidebarSections(page: PageComposition): string[] {
    return page.sections
      .filter(
        (section) =>
          ['quote', 'metric', 'list'].includes(section.type) || section.visualWeight > 72,
      )
      .slice(0, 3)
  ...
  private score(
    archetype: MagazineCompositionPlan['archetype'],
    grid: EditorialGridPlan,
    page: PageComposition,
  ): number {
    let score = 72;
    ...
    if (
      page.sections.some((section) => ['quote', 'metric', 'chart', 'image'].includes(section.type))
    )
      score += 5;
