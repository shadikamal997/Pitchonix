...
export type OutputFormat = 'pptx' | 'pdf' | 'docx' | 'html' | 'md' | 'txt' | 'rtf';
...
export interface ConversionResult {
  ...
  /** Phase Ω.CONTENT.2E — the source UDM (pre-reshape) for content-ledger import. */
  sourceDocument: UniversalDocument;
  /** Source format detected, for ledger metadata. */
...
export class UniversalConversionService {
  ...
  async convert(input: ConvertInput): Promise<ConversionResult> {
    ...
    if (!fmt)
      throw new BadRequestException(
        `Unsupported or undetectable input format (filename=${input.filename}, mime=${input.mimetype})`,
      );
    ...
    // 4) Export.
    const { buffer, mimetype, extension, format } = await this.exportFromUdm(
      reshaped,
      input.targetFormat,
    );
  ...
  /** Phase Ω.CONTENT.2E — public re-parse of any buffer to UDM (for ledger reopen). */
  async parseToUdm(
    buffer: Buffer,
    fmt: SupportedFormat,
    filename = `output.${fmt}`,
  ): Promise<UniversalDocument> {
    return this.importToUdm(buffer, fmt, filename);
  }
  ...
  private async importToUdm(
    ...
    switch (fmt) {
      case 'pdf': {
        ...
        try {
          return await importPdfGeometry(buf);
        } catch {
          return importPdf(buf);
        }
      }
      case 'docx':
      case 'doc':
      ...
      case 'csv':
        return importCsv(buf, filename);
      case 'html':
      case 'htm':
        return importHtml(buf, filename);
      case 'md':
      case 'markdown':
        return importMarkdown(buf, filename);
      case 'txt':
        return importText(buf, filename);
      case 'rtf':
        return importRtf(buf, filename);
      case 'pptx':
      case 'potx':
        return importPptxAsDocument(this.pptxImport, buf);
      default:
    ...
    switch (target) {
      ...
      case 'pdf': {
        ...
        return {
          ...
          format: r.mode === 'libreoffice' ? 'pdf' : 'html',
        };
      }
      default:
  ...
  private buildQualityReport(
    ...
    for (const k of KINDS) {
      ...
      if (a >= b) preserved.push({ kind: k, count: a });
        ...
        modified.push({
          kind: k,
          count: a,
          note: `kept ${a} of ${b} (${Math.round(ratio * 100)}%)`,
        });
    ...
    if (input === 'pptx' && output !== 'pptx') {
      if (beforeCounts['chart'] ?? 0)
        modified.push({
          kind: 'chart-rendering',
          count: beforeCounts['chart'],
          note: 'chart structure preserved as data; visual render is exporter-specific',
        });
      if (beforeCounts['smartArt'] ?? 0)
        lost.push({
          kind: 'smartArt',
          count: beforeCounts['smartArt'],
          note: `flattened to summary text in ${output}`,
        });
    }
    ...
    const safe = (b: number, a: number): number =>
      b === 0 ? 100 : Math.max(0, Math.min(100, Math.round((Math.min(a, b) / b) * 100)));

    const textScore = safe(
      (beforeCounts['paragraph'] || 0) +
        (beforeCounts['heading'] || 0) +
        (beforeCounts['quote'] || 0) +
        (beforeCounts['callout'] || 0) +
        (beforeCounts['code'] || 0),
      (afterCounts['paragraph'] || 0) +
        (afterCounts['heading'] || 0) +
        (afterCounts['quote'] || 0) +
        (afterCounts['callout'] || 0) +
        (afterCounts['code'] || 0),
    );
    ...
    const tableScore = (() => {
      const bt = beforeCounts['table'] || 0;
      const at = afterCounts['table'] || 0;
      if (bt === 0) return 100;
