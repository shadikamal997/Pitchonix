...
function renderNode(n: DocumentNode): string {
  switch (n.type) {
    case 'heading':
      return `<h${Math.min(6, Math.max(1, n.level || 2))}>${esc(n.text || '')}</h${Math.min(6, Math.max(1, n.level || 2))}>`;
    case 'paragraph':
      return `<p>${escWithRuns(n)}</p>`;
    case 'list': {
      const items = Array.isArray(n.items) ? n.items : [];
      const tag = n.ordered ? 'ol' : 'ul';
      const li = items
        .map((i: any) => `<li>${esc(typeof i === 'string' ? i : i.text)}</li>`)
        .join('');
      return `<${tag}>${li}</${tag}>`;
    }
    case 'quote':
      return `<blockquote>${esc(n.text || '')}</blockquote>`;
    case 'callout':
      return `<div class="callout ${n.callout || 'info'}">${esc(n.text || '')}</div>`;
    case 'code':
      return `<pre><code>${esc(n.text || '')}</code></pre>`;
    case 'image':
      return `<img src="${esc(n.src || '')}" alt="${esc(n.alt || '')}" />`;
    case 'table': {
      const rows = n.rows || [];
    ...
    case 'chart': {
      ...
      if (svg)
        return `<figure class="chart" style="margin:12px 0">${svg}${n.chart?.title ? `<figcaption>${esc(n.chart.title)}</figcaption>` : ''}</figure>`;
