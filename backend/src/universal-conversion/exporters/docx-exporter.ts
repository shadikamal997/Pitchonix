...
export async function exportDocx(doc: UniversalDocument): Promise<Buffer> {
  ...
  forEachNode(doc, (node, pageIdx, nodeIdx) => {
    if (node.type === 'chart' && node.chart) {
      ...
      chartJobs.push(
        renderChartPng(node.chart, 960, 540)
          .then((png) => {
            if (png) chartPngs.set(key, png);
          })
          .catch(() => {
            /* swallow per-chart failure */
          }),
      );
    }
  });
  ...
  doc.pages.forEach((page, pi) => {
    ...
    if (page.title) {
      children.push(
        new Paragraph({
          ...
          children: [
            new TextRun({ text: page.title, color: brand.primary, font: brand.headingFont }),
          ],
        }),
      );
    }
    ...
    if (page.notes) {
      children.push(
        new Paragraph({
          children: [
            new TextRun({ text: `Notes: ${page.notes}`, italics: true, color: brand.secondary }),
          ],
        }),
      );
    }
  ...
  switch (n.type) {
    ...
    case 'paragraph': {
            ...
            children: n.runs.map(
              (r) =>
                new TextRun({
                  ...
                  color:
                    r.color && /^#[0-9A-Fa-f]{6}$/.test(r.color)
                      ? r.color.replace('#', '')
                      : brand?.text,
      ...
      return [
        new Paragraph({
          children: [
            new TextRun({ text: n.text || '', color: brand?.text, font: brand?.bodyFont }),
          ],
        }),
      ];
    }
                  ...
                  children: [
                    new Paragraph({
                      children: [
                        new TextRun({
                          text: c.text || '',
                          bold: c.bold || (rIdx === 0 && !!n.headerRow),
                        }),
                      ],
                      alignment:
                        c.align === 'center'
    ...
    case 'image': {
      ...
      return [
        new Paragraph({
          children: [
            new TextRun({
              text: `[Image: ${n.alt || n.src || 'untitled'}]`,
              italics: true,
              color: '64748B',
            }),
          ],
