...
export async function importPptxAsDocument(
  ...
  if (firstResolved) {
    doc.theme = {
      ...
      fonts: {
        heading: parsed.themes[0]?.tokens?.fonts?.heading,
        body: parsed.themes[0]?.tokens?.fonts?.body,
      },
    };
  }
  if (parsed.themes[0]?.name)
    doc.metadata.description = `Imported from theme "${parsed.themes[0].name}"`;
  return doc;
}
...
function mapElementToNode(el: any): DocumentNode | null {
  switch (el.type) {
    case 'heading':
      return { type: 'heading', level: 1, text: el.content?.text, runs: el.content?.runs };
    case 'subheading':
      return { type: 'heading', level: 2, text: el.content?.text, runs: el.content?.runs };
    case 'paragraph':
      return { type: 'paragraph', text: el.content?.text, runs: el.content?.runs };
    case 'image':
      return { type: 'image', src: el.content?.url || el.content?.src || '', alt: el.content?.alt };
    case 'chart':
      return { type: 'chart', chart: el.content };
    case 'table':
      return {
        type: 'table',
        rows: [
    ...
    case 'smartArt':
      return {
        type: 'paragraph',
        text: `[SmartArt: ${el.content?.kind || 'custom'} with ${el.content?.nodeCount || 0} nodes]`,
      };
    case 'oleObject':
      return {
        type: 'callout',
        callout: 'info',
        text: `Embedded ${el.content?.kind || 'object'} attachment: ${el.content?.filename || ''}`,
      };
    case 'shape':
      return null; // visual-only shapes don't translate to documents
    default:
      return el.content?.text ? { type: 'paragraph', text: el.content.text } : null;
  }
}
