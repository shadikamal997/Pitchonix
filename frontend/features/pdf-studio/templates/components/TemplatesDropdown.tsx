...
import { TEMPLATE_REGISTRY, getTemplate } from '../registry/templateRegistry';
import { TemplatePreviewCard } from './TemplatePreviewCard';

const CATEGORIES = [
  'All', 'Business', 'Analytics', 'Sales', 'Strategy', 'Product & Tech', 'Marketing', 'HR & Ops',
];
...
export function TemplatesDropdown({
  ...
  const visible = useMemo(() => {
    ...
    return TEMPLATE_REGISTRY.filter(t => {
      ...
      const qMatch = !q || [t.name, t.category, t.description, ...t.tags].join(' ').toLowerCase().includes(q);
            ...
            <div className="shrink-0 flex items-center justify-between border-b border-gray-100 px-3 py-2">
              <div>
                ...
                <div className="text-[11px] text-gray-500">
                  {visible.length} of {TEMPLATE_REGISTRY.length} designs
                </div>
              </div>
              {selected && (
                <div className="flex items-center gap-1.5">
                  <span className="max-w-[140px] truncate text-right text-[11px] font-medium text-gray-500">
                    {selected.name}
                  </span>
                  <div
                    className="h-2 w-2 rounded-full shrink-0"
                    style={{ background: selected.colors.accent }}
                  />
                </div>
              )}
            </div>
