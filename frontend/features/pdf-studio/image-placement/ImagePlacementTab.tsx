...
export function ImagePlacementTab({
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-gray-700">Selected Image</span>
          <button onClick={onDeselect} className="text-[10px] text-gray-400 hover:text-gray-700 font-medium">
            Deselect
          </button>
        </div>

        {/* Thumbnail */}
        <div className="relative rounded-lg overflow-hidden border border-gray-200 bg-gray-50" style={{ height: 72 }}>
          <img src={selectedImage.url} alt="" className="w-full h-full object-cover" />
        </div>

        {/* Position & Size */}
        <div>
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Position &amp; Size</p>
            ...
            {[
              ...
              <label key={key} className="block">
                ...
                <input
                  ...
                  className="mt-0.5 w-full rounded border border-gray-200 px-2 py-1 text-xs text-gray-800 outline-none focus:border-blue-400"
                />
              </label>
            ))}
          </div>
        ...
        {/* Alignment shortcuts */}
        <div>
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Align Horizontally</p>
          <div className="flex gap-1">
            {[
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1 rounded-lg border border-gray-200 py-1.5 text-[10px] font-semibold text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors"
              >
          ...
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-2 mb-1.5">Align Vertically</p>
          <div className="flex gap-1">
            {[
              { label: 'Top', y: 0 },
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center rounded-lg border border-gray-200 py-1.5 text-[10px] font-semibold text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors"
              >
                {label}
              </button>
            ))}
        ...
        {/* Size presets */}
        <div>
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Size Presets</p>
          <div className="grid grid-cols-3 gap-1">
            {[
              ...
              <button
                ...
                className="rounded border border-gray-200 py-1 text-[10px] font-semibold text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors"
              >
                {preset.label}
              </button>
            ))}
        ...
        {/* Object fit */}
        <div>
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Object Fit</p>
          <div className="flex gap-1">
        ...
        {/* Layer */}
        <div>
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Layer (Z-Index)</p>
          <div className="flex gap-1">
            {[
        ...
        <button
          ...
          className="w-full rounded-lg border border-red-200 bg-red-50 py-2 text-xs font-semibold text-red-600 hover:bg-red-100 transition-colors flex items-center justify-center gap-1.5"
        >
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-bold text-gray-700">Images on this page</p>
        <span className="text-[10px] text-gray-400">Drag on canvas to move</span>
      </div>
      ...
      >
        {uploading ? (
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs font-semibold text-gray-600">Upload image to page</p>
            <p className="text-[10px] text-gray-400">PNG · JPG · WebP · max 10 MB</p>
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="flex w-full items-center justify-between text-xs text-blue-600 hover:text-blue-800 font-semibold"
      >
      ...
      {showGallery && (
        <div className="grid grid-cols-3 gap-1.5">
          {recentImages.length === 0 ? (
            <p className="col-span-3 text-[10px] text-gray-400 text-center py-3">No images yet.</p>
          ) : (
            recentImages.map(img => (
              <button
                key={img.id}
                onClick={() => onAddImage(resolveUrl(img.url))}
                className="relative overflow-hidden rounded-lg border-2 border-transparent hover:border-blue-400 transition-all group"
              ...
              >
                ...
                <div className="absolute inset-0 bg-blue-600/0 group-hover:bg-blue-600/20 flex items-center justify-center transition-all">
                  <span className="opacity-0 group-hover:opacity-100 text-[9px] font-bold text-white bg-blue-600 rounded px-1.5 py-0.5 transition-opacity">
                    Add
                  </span>
                </div>
              </button>
      ...
      <div className="rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-[10px] text-gray-500">
