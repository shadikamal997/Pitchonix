...
const BACKEND_BLUEPRINT_TEMPLATE_IDS = new Set([
...
function usesBackendBlueprint(templateId: string | null | undefined): boolean {
  return BACKEND_BLUEPRINT_TEMPLATE_IDS.has(templateId || '');
}
...
export const SlideCanvas: React.FC<SlideCanvasProps> = ({
  ...
  const backendBlueprintTemplate = usesBackendBlueprint(compositionFamilyId);
  const family = useMemo(
    () => backendBlueprintTemplate ? null : findCompositionFamily(compositionFamilyId),
    [compositionFamilyId, backendBlueprintTemplate],
  );
  ...
  const renderPlan = useMemo(() => {
    ...
    // Premium presentation templates are generated by the backend as final
    // absolute blueprints. Their cards, text, metrics, and accents are
    // intentionally layered. The generic collision repair sees those layers as
    // overlaps and pushes labels/values out of their designed containers.
    // Preserve authored template blueprints exactly; keep composition slotting
    // for legacy smart-family previews only.
    if (backendBlueprintTemplate) {
      return { byId: new Map(planned.map((el) => [el.id, { ...el }])), issues: [] };
    }
    ...
    return { byId, issues };
  }, [sorted, slotMap, backendBlueprintTemplate]);
  ...
  return (
    ...
    >
      ...
      {sorted.map((el) => {
        ...
        const slot = slotMap.get(el.id);
        const useSlot = !!slot;
        // Template families may not have a named slot for every generated
        // component. Those unmatched elements must still render; renderPlan will
        // clamp/stack them instead of silently dropping user or AI content.
