...
export const LayoutSwitcher: React.FC<Props> = ({ currentLayoutId, onPick }) => {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <LayoutGrid className="w-3.5 h-3.5 text-[#4F7563]" />
        {current ? current.name : 'Layout'}
        <ChevronDown className={`w-3 h-3 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="absolute left-0 top-full mt-2 w-[260px] bg-white border border-[#E3E1DA] rounded-lg shadow-2xl z-50 max-h-[460px] overflow-y-auto">
          <div className="px-3 py-2 border-b border-[#F1F0EC]">
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#9A9A9A]">Layout</p>
            <p className="text-[11px] text-[#C9C6BD] leading-snug mt-0.5">Content is preserved — only positions change.</p>
          </div>
          ...
          {(['Basic', 'Visual', 'Data', 'Storytelling'] as const).map((cat) => {
            ...
            return (
              <div key={cat} className="py-1.5 border-b border-[#F1F0EC] last:border-b-0">
                <div className="px-3 text-[10px] font-bold uppercase tracking-wider text-[#9A9A9A] mb-1">{cat}</div>
                {items.map((l) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-2.5 py-2 transition-colors ${isCurrent ? 'bg-[#EEF5F1]' : 'hover:bg-[#EDEBE6]'}`}
                    >
                      <LayoutMock layout={l} />
                      <div className="flex-1 min-w-0 text-left">
                        <div className={`text-xs font-semibold ${isCurrent ? 'text-green-800' : 'text-[#111111]'}`}>{l.name}</div>
                        <div className="text-[10px] text-[#9A9A9A] truncate">{l.description}</div>
                      </div>
                      {isCurrent && <Check className="w-3 h-3 text-[#4F7563] flex-shrink-0" />}
                    </button>
                  );
                })}
              </div>
...
const LayoutMock: React.FC<{ layout: LayoutSpec }> = ({ layout }) => {
  ...
  if (layout.blank) {
    return (
      <div className="w-8 h-[18px] flex items-center justify-center border border-[#C9C6BD] rounded bg-white">
        <span className="text-[8px] text-slate-300">∅</span>
      </div>
    );
  }
  return (
    <div className="relative w-8 h-[18px] border border-[#C9C6BD] rounded bg-white flex-shrink-0 overflow-hidden">
      {layout.slots
